...
#s <- read.table('../turk/exp9/cbgR-exp9-subject_information.tsv',header=F)
...
c<-ggplot(d.tidy,aes(x=value,fill=context,color=context))+
 ...
 # theme_blackDisplay()+
  ...
  guides(fill=guide_legend(title="Type"),color=F)+
  xlab("\n value")+
  ylab("posterior density \n")
...
#f<-read.csv('lvRSA_cnts_c2_expts9_12_generic_mh1000_50_alpha1.csv',header=F)
#g<-read.csv('lvRSA_cnts_c3_expts9_12_generic_mh1000_50_alpha1.csv',header=F)
d<-read.csv('lvRSA_2alphas_expts10e_12_generic_mh1000_10a.csv',header=F)
f<-read.csv('lvRSA_2alphas_expts10e_12_generic_mh1000_10b.csv',header=F)
g<-read.csv('lvRSA_2alphas_expts10e_12_generic_mh1000_10c.csv',header=F)
d<-rbind(d,f,g)


d.tidy<-d %>% select(V1,V2,V36,V37) %>%
  rename(phi_tc=V1,
         phi_ip = V2,
         alpha_tc = V36,
         alpha_ip = V37) %>%
...
ggplot(d.tidy,aes(x=value))+geom_histogram()+facet_wrap(~key,scales='free')
...
s<-data.frame(nzhikes=rbeta(10000,3,3),
        eggs = c(rbeta(3000,50,50),rbeta(7000,100,10000)),
              female = c(rbeta(9500,50,50),rbeta(500,100,10000)),
              rare = c(rbeta(3000,100,900),rbeta(7000,100,10000)))

s<-data.frame(bio=rbeta(10000,0.032,0.01),
              acc=rbeta(10000,3,8))

table(round(10*rbeta(1000,3,8)))
...
ggplot(s.tidy,aes(x=value*100, fill=key, color=key)) +
...
ggsave('../../talks/frisem_5815//figures/schematic_priors.png',width=10, height=10)





post.theta = data.frame(x=seq(0,100,10),
                       acc =c(0.1,0.1,0.15,0.2,0.2,0.1,0.05,0.04,0.03,0.01,0.01),
                       bio =c(rep(0.03,8),0.1,0.35,0.05))

post.theta<- post.theta %>% gather(key,value,-x)

p<-data.frame(bio=rbeta(10000,0.04,0.001),
              acc=rbeta(10000,4,7))
p.tidy<-p %>% gather(key,value)

post.theta$key<-factor(post.theta$key,levels = c('bio',"acc"),
                   labels=c("humans breathe oxygen","humans breathe polluted air"))
...
p.tidy$key<-factor(p.tidy$key,
                   labels=c("humans breathe oxygen","humans breathe polluted air"))

#ggplot(s.tidy,aes(x=value))+geom_density()+facet_wrap(~key,scales='free')

ggplot(s.tidy, 
       aes(x=value*100)) +
  geom_density(size=1,adjust=1.5,alpha=0.5, linetype=3, color='#d95f0e')+
  geom_density(data=p.tidy,aes(x=value*100), color='#d95f0e',
               size=1,adjust=1.5,alpha=0.5, linetype=1)+
  geom_hline(yintercept=0.01, color='#2b8cbe', linetype=2)+
  geom_line(data = post.theta, aes(x=x,y = value/10),
            color='#2b8cbe',
            size=1.5,
...
stat <- stat_function(aes(x = x, y = ..y..), 
                      fun = dbeta, colour="black", n = 100,
                      args = list(shape1 = 0.032, shape2 = 0.1))
p + stat
...
ggplot(tc.tidy, aes(x=prevlev,y=value,
  ...
  xlab("\n prevalence")+
  ylab('P(generic is true)\n')+
  coord_fixed(100)+
  theme(legend.key.size=unit(2, "lines"),
        legend.text.align=NULL)
ggsave('../../talks/frisem_5815//figures/schematic_tc_allPrev.png',width=10, height=10)


#ggsave('../../semfest-2015/figures/schematic_priors_TCdata.png',width=10, height=10)

tc.tidy<-tc.tidy %>% filter(prevlev%in%c(10,50))
tc.tidy$prevlev<-factor(tc.tidy$prevlev,labels=c("low","medium"))

tc.sims<- data.frame(prevlev = c(10,30,50,70,90),
              nzhikes=c(0.10, 0.299,0.5,0.70,0.9),        
              eggs=c(0.40, 0.72,0.85,0.93,0.98),
              # acc=c(0.25,0.56,0.75,0.875,0.964),
              #gendered=c(0.30, 0.63, 0.75, 0.87, 0.96),
              female=c(0.10,0.30,0.50,0.70,0.90),
               rare=c(0.67,0.89,0.95,0.98,0.994))

tc.tidy<- gather(tc.sims, key, value, -prevlev)


tc.tidy<-tc.tidy[c(3,5,8,13,17),]
tc.tidy$generic <- factor(c("hikes are cold", "movie theaters are cold",
                      "birds lay eggs",
                     "birds are female","mosquitos carry wNV"),
                     levels=c("mosquitos carry wNV",
                              "birds are female",
                              "birds lay eggs",
                              "hikes are cold", "movie theaters are cold"),
                     labels=c("(10%) mosquitos carry wNV",
                              "(50%) birds are female",
                              "(50%) birds lay eggs",
                              "(50%) hikes are cold", 
                              "(90%) movie theaters are cold"),                     
                     )
tc.tidy$valid<- factor(c(0,1,1,0,1))

ggplot(tc.tidy, aes(x=generic,y=value,
                  fill=generic, group=key,alpha=valid))+
  geom_bar(position=position_dodge(),stat='identity',color='white')+
  scale_alpha_manual(values=c(0.6,1))+
  geom_hline(yintercept=0.5, linetype=1,size=2, color='white')+
 # scale_colour_brewer(type='qual',palette=8)+
  scale_y_continuous(breaks=c(0,0.25,0.5,0.75,1),limits=c(0,1))+
#  scale_x_continuous(breaks=c(10,30,50,70,90),limits=c(0,100))+
  scale_color_brewer(palette='Set3',type='qual')+
  scale_fill_manual(values=c("#fb8072","#bebada",
                             "#ffffb3","#8dd3c7","#8dd3c7"))+
  theme_blackDisplay()+ 
  guides(fill=guide_legend(title="Generic statement"))+
  xlab("")+
  ylab('\n P(saying generic)')+
  #coord_fixed(100)+
  theme(legend.key.size=unit(2, "lines"),
...
ggsave('~/Dropbox/talks/frisem_5815//figures/schematic_tc.png',width=16, height=8)
...
ggplot(asym.tidy, aes(x=task,y=value,fill=key,alpha=task))+
  ...
  guides(fill = guide_legend(title = 'Property',order = 2), 
...
# s<-data.frame(hobbits=rnorm(10000,mean=5.8,sd=0.5),
#               
#               ents = rnorm(10000,mean=20,sd=1.5),
#               creatures = rnorm(10000, mean = 10, sd = 5))

s<-data.frame(people=rnorm(10000,mean=5.8,sd=0.5),
              buildings = c(100*rbeta(10000,shape1=3,shape2=5),
                1000*rbeta(10000,shape1=1,shape2=3))
                )
...
ggsave('~/Documents/research/generics/talks/frisem_5815/figures/schematic_height.png',width=10, height=7)
...
# Accidental posterior predictives
...
ggplot(d.tidy,aes(x=value))+geom_histogram()+facet_wrap(~key)

d.tidy <- d %>%
  select(V50,V51,V52,V53,V54,V55,V56,V57,V58,V59) %>%
  rename(IMP_10= V50,
         IMP_20=V51,
         IMP_30 = V52,
         IMP_40 = V53,
...
ggplot(d.tidy,aes(x=prevalence,y=mn))+geom_bar(stat='identity',
                                               position=position_dodge())


```

```{r acc.bootstrap.emprior}

d<-read.csv('bootstrap//truthconditions_exp1_S2_accidental_bootstrap_alpha1_alpha2.csv',header=F)

d.tidy <- d %>% select(V7, V8) %>%
  rename(truth_conditions = V7,
         implied_prevalence = V8) %>%
  gather(task, response) %>%
  group_by(task) %>%
  summarise(mn=mean(response),
          CIlow=quantile(response,probs=0.025),
      CIhigh=quantile(response,probs=0.975))
...
ggplot(d.tidy, aes(x=task,y=mn,alpha=task, group=task))+
