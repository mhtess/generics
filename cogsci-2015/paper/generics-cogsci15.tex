...
%from noah 1/30 am:
%-are there ten or eleven values for guess in the prevalence task?
%-we don't include a cost/prior for the generic utterance for S2, right? i removed P(g).
%-we ended up fixing alpha to 1, right? i removed it.
...
%-i think you should run another 70 people in expt 2 this morning. (there's no reason to settle for noisy data where it matters.) make sure you are restricting to us ip addresses (and ideally collecting native language).
%-add CIs to expt 2 plot.
...
In Experiment 2, we \red{verified} that participants' beliefs about the prior on prevalence varied in this way. 
