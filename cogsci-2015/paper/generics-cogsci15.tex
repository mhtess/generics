...
\begin{figure}
\centering
    \includegraphics[width=\columnwidth]{exp1data}
...
Results are shown in Fig~\ref{fig:exp1} (left). We entered participant's truth judgments into a mixed effects logistic regression with random by-item and by-participant effects of intercept and fixed effects of prevalence and context as well as an interaction term\footnote{This was the maximal mixed effect structure supported by the data.}.  
%
Our results replicated the finding of CBG that the generic statement was endorsed more in the dangerous and distinctive (DD) context than in the plain (P) context (Figure \ref{fig:exp1}, left; $\beta=1.99; SE = .36; z = 5.52; p < .001$). 
%
...
The prevalence scores from each task were entered into a linear mixed model with a by-participant random effect of intercept; the fixed effects were context and task with an interaction term. Our results replicated the asymmetry finding of CBG that the generic statement was interpreted as having a higher prevalence than its truth conditions would imply (i.e. main effect of task; $t(105.8) = 6.632; p < 0.001$; see Fig.~\ref{fig:exp1}, right).
...
Qualitatively, they are each bi-modal and the \emph{DD} prior has lower mean.
