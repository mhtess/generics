...
Following the advice of \citeA{Degen2014}, we will model the \emph{implied prevalence} task as a pragmatic listener ($L_{1}$) task.
\red{possibly change last sentence if we switch to speaker model...}  
The CBG \emph{truth conditions} task is modeled slightly differently. It is best understood as a production task, and as such, we model it with a speaker $S_{2}$ who is trying to convey the prevalence to a pragmatic listener, but can only say the generic or its negation (i.e.~yes or no to the generics truth):
\begin{equation} 
...
The speaker in \eqref{eq:S2}, like $L_{1}$, doesn't know the threshold, but knows that $L_{1}$ is thinking about it, and marginalizes over possible $\theta$'s.
