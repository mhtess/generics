...
d$truth_num <- as.numeric(d$truth_conditions=='TRUE')

d.subj<-d %>% 
  select(workerid,stim_type,trialNum, stim_prevalence,response) %>%
  rename(type = stim_type,
         trial = trialNum,
         prev = stim_prevalence) %>%
  mutate(boolresp = as.numeric(response)-1) %>%
  group_by(workerid, type, prev) %>%
  summarise(ave_resp = mean(boolresp))

ggplot(d.subj,aes(x=prev,y=ave_resp, color=type))+
...
d.tidy<- d %>%
  ...
  summarise(p=mean(truth_num),
            n=length(truth_num)) %>%
  mutate(sdev = sqrt(p*(1-p)))

(filter(d.tidy,stim_type=='danger-distinct' & stim_prevalence==10)$p-
  filter(d.tidy,stim_type=='bare' & stim_prevalence==10)$p)/
  sqrt(((filter(d.tidy,stim_type=='danger-distinct' & stim_prevalence==10)$sdev)^2+
     (filter(d.tidy,stim_type=='bare' & stim_prevalence==10)$sdev)^2)/2)

propes(filter(d.tidy,stim_type=='bare' & stim_prevalence==10)$p,
       filter(d.tidy,stim_type=='danger-distinct' & stim_prevalence==10)$p,
       filter(d.tidy,stim_type=='bare' & stim_prevalence==10)$n,
       filter(d.tidy,stim_type=='danger-distinct' & stim_prevalence==10)$n)

tc.bs<-bootstrap.ci.tc(d)

tc.bs$stim_type<-factor(tc.bs$stim_type,
...
d1<-read.csv('cbgR-exp10e-trials.csv')
d1$workerid <- max(d$workerid) + d1$workerid + 1
...
#d <- d %>% filter(!(workerid%in%c(5,12,29,83,80)))

d.tidy <- d %>% select(stim_type, rt, workerid, trialNum, stim_prevalence,
                       response) %>%
  rename(type=stim_type,
         prev = stim_prevalence) %>%
  mutate(boolresp = as.numeric(response=='True')) %>%
  select(-workerid, -rt, -trialNum, -response) 



bootstrap.ci.tc <- function(x){
  agr = aggregate(boolresp ~ prev + type, data=x, FUN=mean)
  agr$CILow = aggregate(boolresp ~ prev+ type, data=x, FUN=ci.low)$boolresp
  agr$CIHigh = aggregate(boolresp ~ prev+ type, data=x, FUN=ci.high)$boolresp
  agr$YMin = agr$boolresp - agr$CILow
  agr$YMax = agr$boolresp + agr$CIHigh
  return(agr)
}

d.bs<- bootstrap.ci.tc(d.tidy)
...
d.subj<- d %>%
  group_by(workerid, stim_prevalence, stim_type) %>%
  summarise(mean_resp = mean(truth_num))

ggplot(d.subj,aes(x=stim_prevalence,y=mean_resp,color=stim_type))+
...
d.bs$type<-factor(d.bs$type,levels=c('danger','distinct','bare'))

ggplot(d.bs, aes(x=prev,y=boolresp,
                  colour=type, group=type))+
  geom_point(size=4, position=position_dodge(5))+
  geom_line(size=0.7,position=position_dodge(5))+
  geom_errorbar(aes(ymin=YMin,ymax=YMax,colour=type),
              width=5,
              size=1,
...
DD, NN, Plain. 1 response / trial. n = 30.

```{r priors1b}

setwd("~/Documents/research/generics/cbg2010-replication/data")
d<-read.csv('cbg-prior1a-trials.csv',header=T)
d$item <- paste(d$stim_color,d$stim_part,sep="_")
table(d$stim_type,d$item)
table(d$stim_type,d$stim_part)
...
### Does the 1st response in the 5 response look like this?
...
d.tidy<-d%>%   select(workerid,rt,
              ...
              stim_type, trial_num) %>%
  rename(context=stim_type) %>%
  gather(option,value,-workerid,-rt,-context, -trial_num)

ggplot(filter(d.tidy, to.n(workerid) <30), aes(x=value, fill=context))+
  geom_histogram(binwidth=1)+
  facet_grid(workerid~trial_num)+
  guides(fill=F)+
  scale_fill_brewer(type='qual',palette=6)+
  xlab('prevalence')+
  ggtitle('human elicited prior distribution')
...
Separate Dangerous, Distinct, Plain. 5 response / trial. n = 30.

```{r priors0b}

setwd("~/Documents/research/generics/cbg2010-replication/data")
d<-read.csv('cbg-prior0b-trials.csv',header=T)
d$item <- paste(d$stim_color,d$stim_part,sep="_")
table(d$stim_type,d$item)
table(d$stim_type,d$stim_part)
...
With n = 30. The histogram looks like *danger* is somewhere in between distinct and bare. The only tests that are significant are for *distinct* vs. *bare*. Still a robust peak at 20%. More subjects?


## Prior1b

Separate Dangerous, Distinct, Plain. 1 response / trial. n = 30.
...
dd<-d%>% 
  filter(stim_type=='nondistinctive') %>%
  mutate(roundval=round(response0/10)*10)
table(dd$roundval)
...
d.tidy %>% group_by(context) %>% summarise(mean = mean(value),
                                           ci = 2*sem(value))
...
# lvRSA using empirical priors


```{r lvRSA.empiricalpriors}

setwd("~/Documents/research/generics/cbg2010-replication/models/bayesian_analysis")
d<- read.csv('lvRSAemprior_2S2_2phis_2alphas_sepDD_mh1000_10b.csv', header=F)


m.tidy<-d %>% 
  select(V1,V2,V182, V183)%>%
  rename(phi_TC=V1,
         phi_IP=V2,
         alpha_TC=V182,
         alpha_IP=V183)%>%
  gather(variable,value)%>%
  separate(variable,into=c("task","parameter"),by="_")


ggplot(m.tidy,aes(x=value))+
  geom_histogram()+
  facet_grid(task~parameter)

```

```{r lvrsa.empiricalpriors.truthconds}
d.tidy<- d %>%
  select(V6,V13,V20,V27,V34,
         V42,V49,V56,V63,V70,
         V78,V85,V92,V99,V106,
        V132,V123,V124,V125,V126,V127,V128,V129,V130,V131,
         V155,V146,V147,V148,V149,V150,V151,V152,V153,V154,
         V178,V169,V170,V171,V172,V173,V174,V175,V176,V177) %>%
  rename(danger_truth_10= V6,
         danger_truth_30= V13,
         danger_truth_50 = V20,
         danger_truth_70 = V27,
         danger_truth_90 = V34,
         nondistinct_truth_10 = V42,
         nondistinct_truth_30 = V49,
         nondistinct_truth_50 = V56,
         nondistinct_truth_70 = V63,
         nondistinct_truth_90 = V70,
         bare_truth_10 = V78,
         bare_truth_30= V85,
         bare_truth_50 = V92,
         bare_truth_70 = V99,
         bare_truth_90 = V106,
        danger_implied_10=V123,
         danger_implied_20=V124,
         danger_implied_30=V125,
         danger_implied_40=V126,
         danger_implied_50=V127,
         danger_implied_60=V128,
         danger_implied_70=V129,
         danger_implied_80=V130,
         danger_implied_90=V131,
         danger_implied_100=V132,         
         nondistinct_implied_10 = V146,
         nondistinct_implied_20 = V147,
         nondistinct_implied_30 = V148,
         nondistinct_implied_40 = V149,
         nondistinct_implied_50 = V150,
         nondistinct_implied_60 = V151,
         nondistinct_implied_70 = V152,
         nondistinct_implied_80 = V153,
         nondistinct_implied_90 = V154,
         nondistinct_implied_100 = V155,
         bare_implied_10 = V169,
         bare_implied_20 = V170,
        bare_implied_30 = V171,
        bare_implied_40 = V172,
        bare_implied_50 = V173,
        bare_implied_60 = V174,
        bare_implied_70 = V175,
        bare_implied_80 = V176,
        bare_implied_90 = V177,
        bare_implied_100 = V178) 


d.stats<- d.tidy %>%
  gather(variable,value) %>%
  separate(variable,into=c('context','qud','prevalence') ,sep="_") %>%
  group_by(context,qud,prevalence) %>%
  summarise(endorsement=mean(value),
        CIlow = quantile(value,probs=0.025),
      CIhigh = quantile(value,probs=0.975)) %>%
  mutate(prevalence=to.n(prevalence))


ggplot(data=filter(d.stats,qud=='truth'),
       aes(x=prevalence,y=endorsement,
...
All of this may  be confusing code / redudant with cogsci-generics.Rmd
...
m$context<-factor(m$context,
                        levels=c("dangerous& distinctive", 
                                 "nondistinctive & irrelevant",
                                 "plain"),
                  labels=c("dangerous & distinctive", 
                                 "nondistinctive & irrelevant",
...
ggsave(filename=paste(plotpath,'exp1data.png',sep=''),
       plot=p3,
       width=14,
       height=6)
...
d<-read.csv('lvRSAemprior_2phis__mh1000_10.csv',header=F)
d<-rbind(d,f,g)
...
lv.alpha<-d %>% select(V182) %>% rename(alpha=V182)
...
to.n(seq(0.01,0.99,0.03))
prob = c(0.53,0.01,0.001,0.22,0.0058,0.0032,0.0024,0.0020,0.0018,0.0018,0.002,0.002,0.0032)
...
mosquitos.data<-data.frame(t(rmultinom(10, 
                                       10, 
                                       c(0.53,0.22,0.0058,0.0032,
                                         0.0024,0.0020,0.0018,
                                         0.0018,0.002,0.002,0.0032))))
...
acc.data<-data.frame(t(rmultinom(100, 
                                   1, 
                                       discretize_beta(0.4,3))))
...
ggplot(m.hist, aes(x=prev,color=context)) +
...
a<-
  ...
  ggplot(schematic.priors,aes(x=100*bin,y=prob,fill=context))+
  geom_bar(stat='identity',position=position_dodge())+
#    geom_smooth(aes(x=100*bin,y=prob, colour=context), 
#              se=F, method="loess",show_guide = FALSE,lwd=0.7) +
  #xlim(0,100)+
#  facet_wrap(~context)+
 # guides(fill=F)+
  scale_x_continuous(breaks=c(0,20,40,60,80,100))+
...
a = data.frame(A=c(0,0,0,0,0,10,10,10,0,0))
ggplot(a, aes(x=A)) +
  geom_line(aes(y = ..density..), stat = 'density') + 
...
Simulations of theoretical interest (e.g. mosquitos carry west nile virus)
...
d<-read.csv('lvRSA_2alphas_expts10e_12_generic_mh1000_10.csv',header=F)

d.phi<- d %>% select(V1,V2) %>% rename(phi_tc = V1, phi_ip = V2) %>%
  gather(key,value)

ggplot(d.phi,aes(x=value))+geom_histogram()+facet_wrap(~key)

d.alpha<- d %>% select(V36,V37) %>% rename(alpha_tc = V36, alpha_ip = V37) %>%
  gather(key,value)

ggplot(d.alpha,aes(x=value))+geom_histogram()+facet_wrap(~key)
...
lvRSA Post pred on asymmetry (new, clean)

```{r postpred.newasym.lvrsa}
...
d.tidy <- d %>%
  ...
  rename(dd_10 = V13,
         ...
         pl_90 = V29) %>%
  gather(key,value, -no) %>%
  separate(key, into = c("context","prevalence"), by = "_") %>%
  spread(prevalence, value)

simulate_subject<- function(posteriorsamples){
  prevlev<-c(10,10,30,30,50,50,70,70,90,90)
  subj.no<-sample(unique(posteriorsamples$no),1)
  subj.probs<-subset(posteriorsamples,no==subj.no)
  dd.resp = c(rbinom(2,1,subj.probs[1,"10"]),
             rbinom(2,1,subj.probs[1,"30"]),
              rbinom(2,1,subj.probs[1,"50"]),
              rbinom(2,1,subj.probs[1,"70"]),
              rbinom(2,1,subj.probs[1,"90"]))
  dd.ave <- sum(dd.resp*prevlev)/sum(dd.resp)
  
  ni.resp = c(rbinom(2,1,subj.probs[2,"10"]),
             rbinom(2,1,subj.probs[2,"30"]),
              rbinom(2,1,subj.probs[2,"50"]),
              rbinom(2,1,subj.probs[2,"70"]),
              rbinom(2,1,subj.probs[2,"90"]))
  ni.ave <- sum(ni.resp*prevlev)/sum(ni.resp)
  
  pl.resp = c(rbinom(2,1,subj.probs[3,"10"]),
             rbinom(2,1,subj.probs[3,"30"]),
              rbinom(2,1,subj.probs[3,"50"]),
              rbinom(2,1,subj.probs[3,"70"]),
              rbinom(2,1,subj.probs[3,"90"]))
...
for (i in 1:10000){
  simulated.subjs[i,]<-simulate_subject(d.tidy)
}
...
+ prevalence levels: 5 - 95, by 5

```{r exp13}
setwd(dir = '~/Documents/research//generics//cbg2010-replication/turk/exp13/')
d<-read.csv(file = 'cbg-exp13-trials.csv')
table(d$stim_prevalence,d$stim_type)
...
d.tidy <- d %>%
  ...
  mutate(boolresp = as.numeric(response)-1) %>%
  group_by(type,prev) %>%
  summarise(endorse = mean(boolresp),
            sterr = sqrt((mean(boolresp)*(1-mean(boolresp)))/(length(boolresp))))

# 
# ggplot(d.tidy, aes(x=prev, y=endorse,color=type))+
#   geom_point(size=3, position=position_dodge(4))+
#   geom_line(size=0.5,position=position_dodge(4))+
#   geom_errorbar(aes(ymin=endorse-1.96*sterr,ymax=endorse+1.96*sterr,
#                     colour=type),
#               width=5,
#               size=0.8,
#               position=position_dodge(4))
...
d$truth_num = as.numeric(d$response)-1
tc.bs<-bootstrap.ci.tc(d)
...
ggplot(d.subj,aes(x=prev,y=boolresp, color=type))+
  geom_line(stat='identity',position=position_dodge())+
  facet_wrap(~workerid)
...
table(d.tidy0$roundval)

out<-data.frame(table(d.tidy0$context, d.tidy0$roundval))
...
setwd("~/Documents/research/generics/cbg2010-replication/data")
d<-read.csv('cbg-prior1b-trials.csv',header=T)
d$item <- paste(d$stim_color,d$stim_part,sep="_")
#table(d$stim_type,d$item)
#table(d$stim_type,d$stim_part)
...
```
# QQ-Plot of 5 response vs. 1 response


```{r qq.5.1}

d.qq<- left_join(d.tidy0,d.tidy1, by=c("context"))


qqplot(filter(d.tidy0, context=='bare')$value,
       filter(d.tidy1,context=='bare')$value,
       xlab='five responses', ylab='one response')

qqplot(filter(d.tidy0, context=='danger-distinct')$value,
       filter(d.tidy1,context=='danger-distinct')$value,
       xlab='five responses', ylab='one response')

qqplot(filter(d.tidy0, context=='nondistinctive')$value,
       filter(d.tidy1,context=='nondistinctive')$value,
       xlab='five responses', ylab='one response')


qqcor<- function(data1, data2){
  data1<-data1 %>%
    mutate(sx=sort(value))

  data2<-data2 %>%
    mutate(sx=sort(value))
  
  lenx <- length(data1$sx)
  leny <- length(data2$sx)
  if (leny < lenx) {
    sx <- approx(1L:lenx, data1$sx, n = leny)$y
    } else {
      sx<- data1$sx
    }
if (leny > lenx){
  sy <- approx(1L:leny, data2$sx, n = lenx)$y
  } else {
  sy<- data2$sx
  }
  print(cor(sx,sy, method='spearman'))
  print(cor(sx,sy, method='pearson'))
}


qqcor(filter(d.tidy0,context=='bare'),
      filter(d.tidy1,context=='bare'))
qqcor(filter(d.tidy0,context=='danger-distinct'),
      filter(d.tidy1,context=='danger-distinct'))
qqcor(filter(d.tidy0,context=='nondistinctive'),
      filter(d.tidy1,context=='nondistinctive'))
```
...
```{r prior2}
setwd(dir = '~/Documents/research/generics/cbg2010-replication/turk/prior2')
d<-read.csv(file = 'cbg-prior2-trials.csv')
head(d)

d.tidy2<- d%>%
  select(context, starts_with("bin"), -binOrder) %>%
  gather(key, value, -context) %>%
  group_by(context,key) %>%
  summarise(ev = mean(value,na.rm=T),
            half.ci = 1.96*sem(value)) %>%
  separate(key, into=c("meh","bin")) %>%
  select(-meh) %>%
  mutate(bin = as.numeric(bin))

ggplot(d.tidy2,aes(x=bin,y=ev))+
  geom_bar(position=position_dodge(),stat='identity')+
  facet_wrap(~context)+
    geom_errorbar(aes(ymin=ev-half.ci,ymax=ev+half.ci),
              width=5,
              size=0.8,
              position=position_dodge())

d.subj<-d%>%
  select(workerid, context, starts_with("bin"), -binOrder) %>%
  gather(key, value, -context, -workerid) %>%
  group_by(workerid,context,key) %>%
  summarise(ev = mean(value,na.rm=T)) %>%
  separate(key, into=c("meh","bin")) %>%
  select(-meh) %>%
  mutate(bin = as.numeric(bin))

# ggplot(d.subj,aes(x=bin,y=ev))+
#   geom_bar(position=position_dodge(),stat='identity')+
#   facet_grid(context~workerid)
```
...
```{r prior3}
setwd(dir = '~/Documents/research/generics/cbg2010-replication/turk/prior3')
d<-read.csv(file = 'cbg-prior3-trials.csv')
#head(d)
d.tidy3<- d%>%
  select(stim_type, starts_with("response")) %>%
  rename(context = stim_type) %>%
  gather(key, value, -context) %>%
...
d.tidy0$type<-factor(d.tidy0$type,
                     levels=c("dangerdistinct","nondistinct","plain"))
...
hist(filter(d.tidy, context=='bare')$value,
     plot=F,breaks=c(0,seq(5,95,10),100))$counts

hist(filter(d.tidy, context=='danger-distinct')$value,
     plot=F,breaks=c(0,seq(5,95,10),100))$counts

hist(filter(d.tidy, context=='nondistinctive')$value,
     plot=F,breaks=c(0,seq(5,95,10),100))$counts

qqcor(filter(d.tidy0,context=='bare'),
      filter(d.tidy3,context=='bare'))
qqcor(filter(d.tidy0,context=='danger-distinct'),
      filter(d.tidy3,context=='danger-distinct'))
qqcor(filter(d.tidy0,context=='nondistinctive'),
      filter(d.tidy3,context=='nondistinctive'))

qqcor(filter(d.tidy3,context=='bare'),
      filter(d.tidy1,context=='bare'))
qqcor(filter(d.tidy3,context=='danger-distinct'),
      filter(d.tidy1,context=='danger-distinct'))
qqcor(filter(d.tidy3,context=='nondistinctive'),
      filter(d.tidy1,context=='nondistinctive'))
...
```{r prior4}
setwd(dir = '../prior4')
d<-read.csv(file = 'cbg-prior4-trials.csv')
#head(d)
d.tidy4<- d%>%
  select(stim_type, starts_with("slider")) %>%
  rename(context = stim_type) %>%
  gather(key, value, -context)

ggplot(d.tidy4,aes(x=value))+
...
qqplot(filter(d.tidy0, context=='bare')$value,
       filter(d.tidy4,stim_type=='bare')$value,
       xlab='five responses', ylab='five sliders')

qqplot(filter(d.tidy0, context=='danger-distinct')$value,
       filter(d.tidy4,context=='danger-distinct')$value,
       xlab='five responses', ylab='five sliders')

qqplot(filter(d.tidy0, context=='nondistinctive')$value,
       filter(d.tidy4,context=='nondistinctive')$value,
       xlab='five responses', ylab='five sliders')




hist(filter(d.tidy, stim_type=='bare')$value,
     plot=F,breaks=c(0,seq(0.05,.95,.10),1))$counts

hist(filter(d.tidy, stim_type=='danger-distinct')$value,
     plot=F,breaks=c(0,seq(0.05,.95,.10),1))$counts

hist(filter(d.tidy, stim_type=='nondistinctive')$value,
     plot=F,breaks=c(0,seq(0.05,.95,.10),1))$counts
...
qqcor(filter(d.tidy0,context=='bare'),
      filter(d.tidy4,context=='bare'))
qqcor(filter(d.tidy0,context=='danger-distinct'),
      filter(d.tidy4,context=='danger-distinct'))
qqcor(filter(d.tidy0,context=='nondistinctive'),
      filter(d.tidy4,context=='nondistinctive'))

qqcor(filter(d.tidy4,context=='bare'),
      filter(d.tidy1,context=='bare'))
qqcor(filter(d.tidy4,context=='danger-distinct'),
      filter(d.tidy1,context=='danger-distinct'))
qqcor(filter(d.tidy4,context=='nondistinctive'),
      filter(d.tidy1,context=='nondistinctive'))
...
qq.summary<-data.frame(spearman=c(0.967,
0.905,
0.973,
0.982,
0.953,
...
0.775,
0.911))
qq.tidy<- qq.summary %>% gather(key,value)

qq.tidy %>% group_by(key) %>%
  summarise(mean(value),
            1.96*sem(value))

qplot(data=qq.tidy,x=value, geom='histogram')+
  facet_wrap(~key)
```

# Density smoothed priors / histogrammed priors for model

```{r densmoothprior.for.model}
setwd("~/Documents/research/generics/cbg2010-replication/data")
d<-read.table('cbgR-exp11-trials.tsv',header=T)
de<-read.table('cbgR-exp11e-trials.tsv',header=T)
de$workerid<-de$workerid+length(unique(d$workerid))
...
d.tidy0<-d%>%   select(workerid,
              response0,response1,response2,response3,response4,
              stim_type) %>%
  rename(context=stim_type) %>%
 gather(option,value,-workerid,-context)

setwd("~/Documents/research/generics/cbg2010-replication/data")
d<-read.csv('cbg-prior1a-trials.csv',header=T)
d.tidy1<-d%>%   select(workerid,
              response0,
              stim_type) %>%
  rename(context=stim_type)%>%
  gather(option,value,-workerid,-context) 
...
d<-read.csv(file = 'cbg-prior3-trials.csv')
d.tidy3<- d%>%
  select(workerid,stim_type, starts_with("response")) %>%
  rename(context = stim_type) %>%
  gather(option, value,-workerid, -context)
...
d.tidy4<- d%>%
  ...
  rename(context = stim_type) %>%
  gather(option, value,-workerid, -context)%>%
  mutate(value = value*100) # since sliders report 0 - 1

sample.30.histogram<-function(context.name){
  samp30<-sample(seq(0,max(d.tidy0$workerid)),30)
  d.samp0<- d.tidy0[d.tidy0$workerid%in%samp30,]
  d.samp0<- d.samp0 %>%
  select(context,value)
  
  samp30<-sample(seq(0,max(d.tidy1$workerid)),30)
  d.samp1 <- d.tidy1[d.tidy1$workerid%in%samp30,]
  d.samp1<- d.samp1 %>%
  select(context,value)
  
  samp30<-sample(seq(0,max(d.tidy3$workerid)),30)
  d.samp3<- d.tidy3[d.tidy3$workerid%in%samp30,]
  d.samp3<- d.samp3 %>%
  select(context,value)

  samp30<-sample(seq(0,max(d.tidy4$workerid)),30)
  d.samp4<- d.tidy4[d.tidy4$workerid%in%samp30,]
  d.samp4<- d.samp4 %>%
  select(context,value)
  prior.combined<-bind_rows(list(d.samp0,d.samp1,d.samp3,d.samp4)) %>%
    filter(context==context.name)
  
  prior.combined$roundval<-round(prior.combined$value/10)*10

  return(table(prior.combined$roundval))
...
bare.hist<- data.frame(t(replicate(1000,sample.30.histogram('bare'))))
bare.hist.sum<- bare.hist %>%
  gather(key,value) %>%
  group_by(key) %>%
  summarise(mn = mean(value),
            ciHi = mean(value) + 1.96*sem(value),
            ciLo = mean(value) - 1.96*sem(value)) %>%
  mutate(type='plain')
bare.hist.sum$mn

dd.hist<- data.frame(t(replicate(1000,sample.30.histogram('danger-distinct'))))
dd.hist.sum<- dd.hist %>%
  gather(key,value) %>%
  group_by(key) %>%
  summarise(mn = mean(value),
...
nn.hist<- data.frame(t(replicate(1000,sample.30.histogram('nondistinctive'))))
nn.hist.sum<- nn.hist %>%
  gather(key,value) %>%
  group_by(key) %>%
  summarise(mn = mean(value),
...
pr.sum<- bind_rows(bare.hist.sum, dd.hist.sum, nn.hist.sum) %>%
  mutate(prev = as.numeric(substring(as.character(key),2)))

pr.sum$type<-factor(pr.sum$type,
                    levels=c("dd", "nn","plain"),
       labels=c( "dangerous & distinctive", 
                "nondistinctive & nondangerous","plain"))

ggplot(pr.sum,aes(x=prev,y=mn, fill=type))+
  geom_bar(stat='identity',position=position_dodge())+
  #geom_errorbar(aes(ymin=ciLo,ymax=ciHi),position=position_dodge())+
  facet_wrap(~type)+
  scale_fill_brewer(palette='Set1')+
  ylab('frequency')+
  xlab('prevalence')+
...
smooth.bare<-data.frame(bkde((prior.combined%>%filter(context=='bare'))$value, 
     kernel='normal', gridsize=11, truncate=TRUE, range.x=c(0,101)))

qplot(data=smooth.bare,geom='bar',x=x,y=y,stat='identity',position=position_dodge())



hist((prior.combined%>%filter(context=='danger-distinct'))$value, 
     breaks=11,plot=F)$counts

hist((prior.combined%>%filter(context=='nondistinctive'))$value, 
     breaks=11,plot=F)$counts
```



```{r bootstrap.dd.nn.pl}
m0<-max(d.tidy0$workerid)
d.tidy1$workerid<-d.tidy1$workerid+m0+1
d.tidy3$workerid<-d.tidy3$workerid+max(d.tidy1$workerid)+1
d.tidy4$workerid<-d.tidy4$workerid+max(d.tidy3$workerid)+1
d.bind<-bind_rows(d.tidy0,d.tidy1,d.tidy3,d.tidy4)

d.bind<- d.bind %>%
  mutate(roundval=round(value/10)*10)

n.subj<-length(levels(factor(d.bind$workerid)))

db.total=data.frame()
for (i in 1:1000){
  samp.subj<-sample.int(n.subj,replace=T) 
  samp.tidy<-d.bind[d.bind$workerid%in%samp.subj,]
  df.temp<-data.frame(table(samp.tidy$context,samp.tidy$roundval)+1) %>%
    spread(Var2, Freq)
...
file.path<-'../../models/'
write.csv(db.total, file=paste(file.path,'bootstrap_ddnnp_priors.csv',sep=''))

```

# Experiment 14: Accidental


```{r exp14}
setwd(dir = '~/Documents/research//generics//cbg2010-replication/turk/exp14/')
d<-read.csv(file = 'cbg-exp14-trials.csv')
table(d$stim_prevalence,d$stim_type)
...
d.imp <- d %>%
  ...
  select(workerid,stim_type,trial_num, response) %>%
...
tc.church <- d.tc %>%
  select(prev, boolresp)

write.csv(tc.church,'accidental_tc_long.csv')
imp.church <- d.imp %>%
  select(prev_score)


bootstrap.ci.tc <- function(x){
  agr = aggregate(boolresp ~ prev , data=x, FUN=mean)
  agr$CILow = aggregate(boolresp ~ prev, data=x, FUN=ci.low)$boolresp
  agr$CIHigh = aggregate(boolresp ~ prev, data=x, FUN=ci.high)$boolresp
  agr$YMin = agr$boolresp - agr$CILow
  agr$YMax = agr$boolresp + agr$CIHigh
  return(agr)
}
tc.sum<- bootstrap.ci.tc(d.tc)

ggplot(tc.sum, aes(x=prev,y=boolresp))+
  geom_point(size=3, position=position_dodge(4))+
  geom_line(size=0.5,position=position_dodge(4))+
  geom_errorbar(aes(ymin=YMin,ymax=YMax),
              width=5,
  ...
  scale_x_continuous(breaks=c(10,30,50,70,90),limits=c(0,100))+
  theme_paper()+ 
  xlab("XX% of lorches have muddy feathers")+
  ylab('proportion of "true" responses')+
  coord_fixed(100)
...
d.imp <- d %>%
  ...
  select(workerid,stim_type,trial_num, response) %>%
  group_by(workerid) %>%
  summarise(prev_score = mean(response)) %>%
  mutate(task = "implied")
...
#   select(response) 
# 
# write.csv(imp.church,'../../data/accidental_ip_long.csv')



d.tc.sum <- d.tc %>%
  group_by(workerid) %>%
  summarise(prev_score = sum(prev*boolresp)/sum(boolresp)) %>%
  mutate(task = "truth")

d.tc.sum[is.nan(d.tc.sum$prev_score),]$prev_score=100

d.tasks<-bind_rows(d.tc.sum,d.imp)


bootstrap.ci.ip <- function(x){
  agr = aggregate(prev_score ~ task, data=x, FUN=mean)
  agr$CILow = aggregate(prev_score ~ task, data=x, FUN=ci.low)$prev_score
  agr$CIHigh = aggregate(prev_score ~ task, data=x, FUN=ci.high)$prev_score
  agr$YMin = agr$prev_score - agr$CILow
  agr$YMax = agr$prev_score + agr$CIHigh
  return(agr)
}

d.bs<-bootstrap.ci.ip(d.tasks)
d.bs<- d.bs %>%
  mutate(context = "accidental",
         task = factor(task, levels=c("truth","implied"),
                       labels=c("truth conditions", "implied prevalence"))) %>%
  rename(imp_subj = prev_score)


# dm.bs$context<- factor(dm.bs$context, levels = c("distinctive & dangerous",
#                                                   "nondistinctive & nondangerous",                                                
#                                                   "plain",
#                                                   "accidental"),
#                        labels=c("distinctive &\n dangerous",
#                                 "nondistinctive &\n nondangerous",                                                
#                                                   "plain",
#                                                   "accidental"))
# dm.bs$type<-'k'
# dm.bs[dm.bs$context=='accidental',]$type = 'accidental'
# dm.bs[dm.bs$context!='accidental',]$type = 'biological'
...
f.bs<-bootstrap.ci.ip(f.stats)
f.bs$task<-'impliedprevalence'
e.bs<-bootstrap.ci.ip(d.stats)
e.bs$task<-'truthconditions'

m<-rbind(f.bs,e.bs)

m<-m %>% mutate(task = factor(task, levels=c("truthconditions","impliedprevalence"),
                       labels=c("truth conditions", "implied prevalence")))
dm.bs<-rbind(d.bs,m)
...
dm.bs<-filter(dm.bs, context%in%c("accidental","bare"))
dm.bs$type<-c("accidental","accidental","biological","biological")
dm.bs$type<-factor(dm.bs$type,levels=c("biological","accidental"),
                   labels=c("biological \n (Exp. 1)", "accidental \n (Exp. 3)"))

ggplot(dm.bs, aes(x=type,y=imp_subj,fill=type,alpha=task, group=task))+
  geom_bar(stat='identity',
           position=position_dodge(0.5), width=0.5,color='white')+
  geom_errorbar(aes(ymin=YMin,ymax=YMax),
                width=0.2,
                size=1,
                color='grey',
...
ggplot(data=d.sum, aes(x=Var1,y=prob))+
...
# Experiment 15: Dangerous, distinct, plain (proper randomization)



```{r exp15}
setwd(dir = '~/Documents/research//generics//cbg2010-replication/turk/exp15/')
d<-read.csv(file = 'cbg-exp15-trials.csv')
table(d$stim_prevalence,d$stim_type)
d$item <- paste(d$stim_color,d$stim_part,sep="_")

d.tc <- d %>%
  filter(trial_type=='truth_conditions') %>%
  select(workerid,stim_type,trialNum, stim_prevalence,response, item) %>%
  rename(type = stim_type,
         trial = trialNum,
         prev = stim_prevalence) %>%
  mutate(boolresp = as.numeric(response=='True'))
...
d.tc$prev_centered <- scale(d.tc$prev, scale=F)

#contrasts(d$stim_type) = cbind(quad = c(-1,2,-1), lin = c(-1, 0, 1))
#contrasts(d$stim_type) = cbind(dd = c(0,1,0), ni = c(0, 0, 1))
contrasts(d.tc$type)
rs<-glmer(boolresp ~ prev_centered*type + 
            (1 | workerid) + 
            (1 | item), 
          data=d.tc, family = 'binomial')
...
tc.sum$type<-factor(tc.sum$type, labels=c("plain","danger","distinct"))

ggplot(tc.sum, aes(x=prev,y=boolresp, color=type, group=type))+
  geom_point(size=3, position=position_dodge(6))+
  geom_line(size=0.5,position=position_dodge(6),linetype=3)+
  geom_errorbar(aes(ymin=YMin,ymax=YMax),
              width=5,
              size=0.8,
...
## Separate dangerous , distinct prior stats
...
ggsave('../../../talks/frisem_5815/figures/ddp_10priors.png',width=16, height=6)



```

# Bootstrap priors for model

Sample n_subjects with replacement, create multinomial, write to csv.

```{r bootstrap.priors.for.model}
setwd(dir = '~/Documents/research/generics/cbg2010-replication/turk/prior6')
d<-read.csv(file = 'cbg-prior6-trials.csv')
...
n.subj<-length(levels(factor(d.tidy6$workerid)))


df.total=data.frame()
for (i in 1:1000){
  samp.subj<-sample.int(n.subj,replace=T) 
  samp.tidy<-d.tidy6[d.tidy6$workerid%in%samp.subj,]
  df.temp<-data.frame(table(samp.tidy$context,samp.tidy$roundval)+1) %>%
    spread(Var2, Freq)
  df.total<-bind_rows(df.total,df.temp)
}
file.path<-'../../models/'
write.csv(df.total, file=paste(file.path,'bootstrap_ddp_priors.csv',sep=''))
```


```{r bootstrap.priors.for.model}
setwd(dir = '~/Documents/research/generics/cbg2010-replication/models/bayesian_analysis//bootstrap/')
m<-read.csv(file = 'truthconditions_S2_bootstrap_alpha1.csv',header=F)
m<-read.csv(file = 'truthconditions_exp1_S2_bootstrap_alpha1_alpha2.csv',header=F)
m<-read.csv(file = 'truthconditions_exp1_S2_accidental_bootstrap_alpha1_alpha2.csv',header=F)
...
m.tidy <- m %>%
  ...
  rename(bare_10 = V1,
         ...
         distinct_90 = V17) %>%
  gather(key, value) %>%
  separate(key, into=c("type","prev"), sep="_") %>%
  group_by(type, prev) %>%
  summarise(expected.value = mean(value),
        CIlow=quantile(value,probs=0.025),
      CIhigh=quantile(value,probs=0.975)) %>%
  mutate(prev = to.n(prev))
...
m.tidy$type <- factor(m.tidy$type, levels=c("danger","nondistinct","bare"),               
                      labels =c("dangerous & distinct",
                             "nondangerous & nondistinct", "plain"))



m.tidy$type <- factor(m.tidy$type, 
                      levels=c("bare","danger","distinct"),
                      labels=c("plain","danger","distinct"))


ggplot(data=m.tidy,aes(x=prev,y=expected.value,color=type,group=type))+
  geom_point(size=2.5, position=position_dodge(6))+
  geom_line(size=1,position=position_dodge(6), linetype=3, alpha=1)+
  geom_errorbar(aes(ymin=CIlow,ymax=CIhigh,colour=type),
              width=5,
              size=1,
              position=position_dodge(6))+
 ...
 # theme_blackDisplay()+ 
  ...
  #guides(color=F)+
  xlab("prevalence")+
 # ylab('P("generic is true" | prevalence) \n')+
  coord_fixed(100)+
  theme(legend.key.size=unit(2, "lines"),
        legend.text.align=NULL)+
  xlab("\n prevalence")+
...
ggsave('../../../../talks/frisem_5815/figures/empiricalbootstrap_truthconditions_ddp.png',width=10, height=12)
...
a.tidy<- m %>%
  select(V20, V21, V22, V24, V25, V26) %>%
  rename(tc_plain = V20,
         tc_danger = V21,
         tc_nondistinct = V22,
         ip_plain = V24,
         ...
         ip_nondistinct = V26) %>%
  gather(key, val) %>%
  separate(key, into=c("task","type"), sep="_") %>%
  group_by(task) %>%
  summarise(expected.value = mean(val),
        CIlow=quantile(val,probs=0.025),
      CIhigh=quantile(val,probs=0.975))

a.tidy$type <- 'biological'
...
sum.tidy<-bind_rows(a.tidy,acc.tidy)

sum.tidy$type<-factor(sum.tidy$type,levels=c("biological","accidental"))

#,
 #                  labels=c("biological (Exp. 1)", "accidental (Exp. 3)"))

sum.tidy$task<-factor(sum.tidy$task,levels=c("tc","ip"),
                      labels=c("truth conditions","implied prevalence"))

sum.tidy<-sum.tidy[c(1,2),]

ggplot(sum.tidy, aes(x=type,y=expected.value,
                     fill=type,alpha=task, group=task))+
  geom_bar(stat='identity',
           position=position_dodge(0.5), width=0.5,color='white')+
  geom_errorbar(aes(ymin=CIlow,ymax=CIhigh),
                width=0.2,
...
ggsave('../../../../talks/frisem_5815/figures/empiricalbootstrap_asym_ddnnp_withAcc.png',width=12, height=7)

```

# Experiment 16: Importance

```{r exp16}
setwd(dir = '~/Documents/research//generics//cbg2010-replication/turk/exp16/')
d<-read.csv(file = 'cbg-exp16-trials.csv')
table(d$stim_prevalence,d$stim_type)
d$item <- paste(d$stim_color,d$stim_part,sep="_")

d.tidy <- d %>%
  filter(trial_type=='importance') %>%
  select(workerid,stim_type,trialNum, stim_prevalence,response, item) %>%
  rename(type = stim_type,
         trial = trialNum,
         prev = stim_prevalence)
...
d.item <- d.tidy %>%
  group_by(type,prev,item) %>%
  summarise(ave = mean(response))

ggplot(d.item, aes(x=ave, fill=type))+
  geom_histogram(position=position_dodge(), binwidth=0.025)
   # scale_fill_brewer(type='qual',palette='Dark2')#+
  #geom_density(alpha=0.1, adjust=0.5)


qplot(data=d.tidy,x=response,geom='histogram')+
  facet_grid()

d.ave<-d.tidy %>% group_by(prev,type) %>%
  summarise(ave = mean(response),
            sterr = sem(response))

d.tidy$prev_centered <- scale(d.tidy$prev, scale=F)
rs<-lmer(data=d.tidy, 100*response~prev_centered*type + (1 | workerid))
summary(rs)

d.ave$type<-factor(d.ave$type, labels=c("plain","danger","distinct"))

ggplot(d.ave, aes(x=prev,y=ave, color=type, group=type))+
  geom_point(size=3, position=position_dodge(8))+
  geom_line(size=0.8,position=position_dodge(8),linetype=3)+
