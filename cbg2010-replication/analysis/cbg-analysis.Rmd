...
d<-read.csv(paste('speak_listenDifAlt_betaHyper_discreteHyper_exp9all_generic_truth_conditions_mh100_50_alpha10.csv',sep=''),header=F)
...
sb<-melt(data.frame(dangerdistinct=sample.beta(1000,0.1347,1.2723),
               nondistinctive=sample.beta(1000,0.2992,1.4554),
               bare=sample.beta(1000,0.2040,1.4770)
               ))
...
p<-ggplot(sb,aes(x=100*value,fill=variable))+
  ...
  guides(fill=F)+
  scale_fill_brewer(type='qual',palette=6)+
  xlab("mean inferred prior distribution, from RSA and exp1")
grid.arrange(exp2,p)
...
```{r tfbt.asymmetry}
#setwd('../models/bayesian_analysis/')
d<-read.csv(paste('speak_listenDifAlt_betaHyper_discreteHyper_exp9all_generic_truth_conditions_mh100_10_alpha3.csv',sep=''),header=F)
