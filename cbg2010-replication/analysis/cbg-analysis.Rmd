...
table(d.tidy[,c("context","roundval")])
...
m.tidy<-d %>% 
  ...
  rename(bare_gamma=V1,
         bare_delta=V2,
         dangerdistinct_gamma=V4,
         dangerdistinct_delta=V5,
         nondistinctive_gamma=V7,
         nondistinctive_delta=V8)%>%
  gather(variable,value)%>%
...
rs<-glmer(truth_num ~ stim_prevalence*stim_type + (1 | workerid) + (1 | item), data=d, family = 'binomial')
summary(rs)
...
d<-read.csv('ft_totallyFixed_conditionIndependenly_cnts_expts9_12_generic_mh1000_10_alpha1.csv',header=F)
...
ggplot(ft.phi,aes(x=value,alpha=task))+
  geom_density(fill='black')+
  theme_paper()+
  scale_alpha_manual(values=c(0.4,1))+
  scale_x_continuous(breaks=c(0.25,.5,0.75),limits=c(0,1))+
  scale_fill_brewer(type='qual',palette=1)+
  theme(strip.text.x = element_text(size=14,color='black'),
        legend.position='bottom',
        legend.direction='horizontal')+
...
ggplot(d.tidy,aes(x=value,alpha=task))+
  geom_density(fill='black')+
  theme_paper()+
  scale_alpha_manual(values=c(0.4,1))+
  scale_x_continuous(breaks=c(10,30,50,70,90),limits=c(0,100))+
...
ggplot(phi.tidy,aes(x=value,alpha=task))+
  geom_density(fill='black')+
  theme_paper()+
  scale_alpha_manual(values=c(0.4,1))+
  scale_x_continuous(breaks=c(0.25,.5,0.75),limits=c(0,1))+
...
ggplot(theta.tidy,aes(x=value,fill=context,alpha=task))+
  geom_density()+
  theme_paper()+
    scale_alpha_manual(values=c(0.4,1))+
  scale_x_continuous(breaks=c(10,30,50,70,90),limits=c(0,100))+
...
#ft.phi <- d %>% select(V1) %>% rename(phi=V1)
ft.phi <- d %>% select(V1,V2) %>% rename(phi_tc=V1,phi_ip=V2) %>%
  gather(variable,value) %>% separate(variable,into=c("parameter","task"),
                                      by="_")
...
ggplot(sb,aes(x=100*value,fill=variable))+
  #geom_density()+
  geom_histogram()+
  xlim(0,100)+
  facet_wrap(~variable)+
