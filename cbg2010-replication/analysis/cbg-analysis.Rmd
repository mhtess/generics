...
# Experiment 4 -- within-subjects effect of context on Generic (Replicate truth conditions of Exp 1 of CBG exactly)
...
results<-glmer(truth_num ~ stim_prevalence*stim_type + (1 | workerid), data=td5, family='binomial')
summary(results)

summary(aov(truth_num~(stim_prevalence*stim_type),
    data=td5))
```
...
cntxt.ef5<-ddply(td5, .(workerid), summarise,
      ...
      dangdistinct_seen=cumsum(as.numeric(stim_type=='danger/distinct')),
