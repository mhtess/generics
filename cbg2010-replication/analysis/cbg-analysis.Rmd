...
asym<-ggplot(m, aes(x=context,y=imp_subj,fill=context,alpha=task, group=task))+
...
# TFBT on fixed-threshold, 2 ways

```{r tfbt.ft.paper}

setwd("~/Documents/research/generics/cbg2010-replication/models/bayesian_analysis")
d<-read.csv('ft_condition2_cnts_expts9_12_generic_mh1000_10_alpha1.csv',header=F)


ft.phi <- d %>% select(V1) %>% rename(phi=V1)
qplot(data=ft.phi,x=phi,geom='density')


  d.tidy <- d %>%
  select(V2,V3,V4) %>%
  rename(danger_distinct = V2,
         nondistinct = V3,
         bare = V4) %>%
  gather(variable,value)

ggplot(d.tidy,aes(x=value,fill=variable))+
  geom_density(alpha=0.8)+
  theme_paper()+
  scale_x_continuous(breaks=c(10,30,50,70,90),limits=c(0,100))+
  scale_fill_brewer(type='qual',palette=6)+
...
d<-read.csv('lvRSA_condition2_dscFine_2phis_expts9_12_generic_mh1000_10_alpha1.csv',
  header=F)
