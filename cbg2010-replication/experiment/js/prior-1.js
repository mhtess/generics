 ...
 //     this.stimtype = exp.stimtype[this.stim.list]; // exp.stimtype already randomized, grab which stimtype corresponds to list #_this.stim
    ...
    //  this.stimtype = exp.stimtype[stim_num%10][this.stim.list]
      ...
      // } else{
       ...
       var query_prompt = "What percentage of " + "<em>"+this.stims[0].category+"</em>" + " do you think have <strong>" + 
      ...
      $("#kind").html(utils.upperCaseFirst(this.stims[0].category));  
    ...
    button : function() {
      ...
      var responses=[]
      for (i=0; i<exp.numKinds; i++){
        responses[i] = $("#text_response"+i).val();
      }
      var valid = responses.map(function(x) {return (x<=100 && x>=0 && x!='');})
      ...
      exp.data_trials.push({
        ...
        "response0" : $("#text_response0").val(),
        // "response1" : $("#text_response1").val(),
        // "response2" : $("#text_response2").val(),
        // "response3" : $("#text_response3").val(),
        // "response4" : $("#text_response4").val(),
        // "response5" : $("#text_response5").val(),
        // "response6" : $("#text_response6").val(),
        // "response7" : $("#text_response7").val(),
        // "response8" : $("#text_response8").val(),
        // "response9" : $("#text_response9").val(),
        "rt":this.rt,
        "stim_type": this.stimtype,
        "stim_determiner": this.determiner,
        "stim_category0": this.stims[0].category,
        // "stim_category1": this.stims[1].category,
        // "stim_category2": this.stims[2].category,
        // "stim_category3": this.stims[3].category,
        // "stim_category4": this.stims[4].category,
        // "stim_category5": this.stims[5].category,
        // "stim_category6": this.stims[6].category,
        // "stim_category7": this.stims[7].category,
        // "stim_category8": this.stims[8].category,
        // "stim_category9": this.stims[9].category,
...
//  var contexts = utils.flatten(utils.fillArray(["bare","danger-distinct","nondistinctive"],10));
  var contexts = utils.flatten(utils.fillArray(["bare","danger","distinct"],10));
