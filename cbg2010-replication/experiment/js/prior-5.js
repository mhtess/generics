...
function make_slides(f) {
  ...
  slides.implied_prevalence = slide({
    ...
    present_handle : function(stim_num) {
      ...
      this.determiner = "generic" // exp.determiner between-subjects var
      ...
      this.n_sliders = exp.numKinds;
      $(".slider_row").remove();
      for (var i=0; i<this.n_sliders; i++) {
        var sentence = this.stims[i].category;
      ...
      // exp.sliderPost = [];
      exp.sliderPost = utils.fillArray(-1,exp.numKinds);//[];

      // for (i=0; i<exp.numKinds; i++){
      //   $("#k"+i).html(this.stims[i].category)
      // };
    ...
    log_responses : function() {
      exp.data_trials.push({
        ...
        "slider0": exp.sliderPost[0],
        "slider1": exp.sliderPost[1],
        "slider2": exp.sliderPost[2],
        "slider3": exp.sliderPost[3],
        "slider4": exp.sliderPost[4],
...
//  var contexts = ["bare","danger","distinct","bare","danger","distinct"];
  ...
  exp.numKinds = 5; // number of kinds to display per screen
...
//  exp.determiner = _.shuffle(["generic","some","most"]);
 ...
 // var context_assign = _.shuffle(contexts) // shuffle 6 contexts
  ...
  var shuffled_properties = _.shuffle(accidental)

  for ( var i=0; i< exp.numTrials; i+=1 ){
    flatstims[i*exp.numKinds].accidental = shuffled_properties[i]
  };
