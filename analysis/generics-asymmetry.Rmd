...
d<-read.csv('~/Documents/research/generics/models/data/asymmetry-2-trials-medSplitAccidentalFBT.csv')
...
contrasts(d.implied$type)<- cbind(acc_high = c(1, 0, 0,0,0), 
                                  acc_lo = c(0, 1, 0,0,0), 
                                  vague = c(0, 0, 0,0,1), 
                                  part = c(0, 0, 0,1,0) )

rs1<-lmer(data=d.implied, response~ type + (1 + type | workerid))
rs2<-lmer(data=d.implied, response~ type + (1 | workerid))

#rs3<-lmer(data=d.implied, response~ type + (1 + type | workerid)+
#                                            (1 | stim_property))
summary(rs2)
...
d<-read.csv("~/Documents/research/generics/models/results/generics-asym-prior-incrmh50000.csv")
...
#ggsave(file=paste('~/Box Sync/talks/esslli-2015-generics/figures/asym-phi-',paramsetups,'-',samples/1000,'k.pdf',sep=''))
...
ggplot(d.opt, aes(x=Response))+
  geom_histogram(aes(y=(..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..]),fill='grey70')+
