...
```{r}
model.posteriorpred <-read.csv('generics_truthJudge_prasada1.csv',header=F,
...
ggplot(data=tj.bs, aes(x=sentence, y=response-0.5))+
  ...
  geom_errorbar(aes(ymin=YMin-0.5,ymax=YMax-0.5), width=0.5)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1,
                                   vjust=1))+
  ylim(-.5,0.5)+
  xlab("")+
...
# ggsave(file='../manuscript/figures/cogsci/truhtjudge_n100.png',
#        width=24,height=6)
...
qplot(data=combined.model.data,
  ...
  xlab("model posterior predictive: P(generic)")+
