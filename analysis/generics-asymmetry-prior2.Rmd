...
d.item$stim_property<-factor(d.item$stim_property,
       levels= levels(with((d.item %>% filter(resp=='response0')), reorder(stim_property, mean_response, function(x) -x))))
...
```{r item.fbt}
f<- read.csv('~/Documents/research/generics/analysis/FBT-priors/results/priors_betas_byItem_incrMH100000.csv')
...
f.exp$Item<-factor(f.exp$Item,
       levels= levels(with((f.exp %>% filter(Question=='response1')), reorder(Item, expectation, function(x) -x))))

ggplot(f.exp, aes(x=Item, y = expectation))+
  geom_bar(stat='identity', position=position_dodge())+
  #geom_errorbar(aes(ymin = mean_response-2*sterr, ymax = mean_response+2*sterr))+
  facet_wrap(~Question) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1,
