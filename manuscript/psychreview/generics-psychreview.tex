...
This latter perspective emphasizes the structure of generic knowledge \cite{Prasada2000}, and views generic utterances as the way of expressing special mental relationships between kinds and properties \cite{Leslie2008, Prasada2012}. 
\citeA{Leslie2007} introduces the notion that generics are tied to a ``default mode of generalization''. This default mode includes such knowledge that striking properties (e.g. properties which are dangerous or appalling) concern aspects of the world that it would be useful to be forewarned about. This is the reason why \emph{Mosquitos carry malaria.} seems true: It is a useful bit of information to convey.\footnote{\citeauthor{Leslie2007} goes on to say that it depends also on the fact that non-malaria carrying mosquitos must have the potential to carry malaria, and this is why \emph{Insects carry malaria} or \emph{Animals carry malaria} are not as true.}
For properties that are not striking (e.g. in \emph{Barns are red.}), then statistical information must be consulted: If the majority of K have F, then the generic is judged to be true. Finally, \citeauthor{Leslie2007} invokes the distinction between ``negative'' and ``positive'' counter-instances. A negative counter-instance is one that is characterized by ``not having F'' (e.g. a bird that doesn't lay eggs) whereas a positive counter-instance is one that is characteristic by ``having not-F'' (e.g. a hypothetical bird that bears live young). Here, it is argued that when positive counter-instances exist, we are much less likely to accept the generic (e.g. \emph{Birds are female}, because ``being male'' is a positive counter-instance of ``being female'') than when there are only negative counters-instances, we will accept the generic (e.g. in \emph{Birds lay eggs.}).

\citeA{Prasada2006} distinguish between statistical and principled relations within concepts. 
These distinct relation types manifest themselves as different inferences one could draw about the relation. These include the expectation that the relation will hold for future instances (operationalized using the phrase \emph{In general, Ks have F}), the explanation that the relation holds because of the kind of thing that K is (\emph{Ks have F by virtue of being a K}), and that the principled relation is normative (operationalized using the phrase \emph{Ks should have Fs}).
These different kinds of relations relate to \citeA{Leslie2007}'s distinctions among generics.
Generics like \emph{Birds lay eggs.} (in which only a minority of K have F) exhibit characteristics of principled connections, though striking generics (e.g. \emph{Mosquitos carry malaria}) have causal connections. This is taken as support for Leslie's claims of the different types of default generalizations.
...
Accounts from the tradition of formal semantics tend to look towards precise, quantifiable conditions to determine if a sentence is true or false.
The most promising account in this tradition was put forth in \citeA{Cohen1999}.
To understand this theory, let's introduce some minimal notation:
...
Assuming the generic relates to the property prevalence, then the simplest meaning would similarly be a threshold on prevalence: $P(F\mid K)>\tau$. \citeA{Cohen1999} takes $\tau = 0.5$; that is, \emph{Robins lay eggs} is taken to mean \emph{Most robins lay eggs}. 

Of course most robins do not lay eggs; none of the male robins lay eggs. Cohen's system resolves this issue by introducing certain constraints into the computation of prevalence: $P(F\mid K)$. In particular, prevalence is computed with respect to a \emph{partition set}. In this theory, \emph{lays eggs} induces a set of alternatives that all have to do with procreation (e.g. \emph{gives birth to live young}, \emph{undergo mitosis}, ...). Then, the individuals that enter into the prevalence computation are only those individuals that would satisfy one or another alternative. \emph{Birds lay eggs} is taken to mean \emph{Most female birds lay eggs}. 

Though the theory is precise, it relies upon a number of a working parts that seem to be determined in some (unspecified) way by pragmatics. 
In addition, it is not clear what this semantic theory relates to the conceptual accounts of generics, and why it is that generic language is so ubiquitous and early acquired.

We aim to unify these two disparate ways of understanding generic language. 
We begin with the intuition that generic language is not unique in its flexibility.
...
We follow \citeA{Cohen1999} and adopt the simplest meaning for generic statements: a similar threshold on prevalence: $P(F\mid K)>\tau$.\footnote{Because we aim to explain the psycholinguistics of generics, we are generally interested in the subjective probability, not the actual frequency in the world.}
...
The goal of this section is introduce the reader to the Rational Speech-Acts framework and highlight relevant applications when appropriate.

Language understanding can be viewed as a special case of social cognition instantiated in a Bayesian model \cite{Goodman2013}. 
In this view, an utterance is \emph{interpreted} by a listener consulting her intuitive theory of speech \emph{production}, i.e. her theory of a speaker. This intuitive theory of a speaker has the assumption that speakers choose utterances approximately optimally on the basis of certain goals, specifically conveying information to the listener. 
A listener then ``inverts'' her model of a speaker and, combined with other relevant world knowledge, arrives at a pragmatic interpretation of an utterance. 
...
Prior beliefs about the world $P(x)$ capture relevant world-knowledge, which is important for deriving context-specific interpretations \red{(cite Degen, Tessler, Goodman cogsci?)}.
The production model $P_{S}$ is defined recursively with the listener model: The speaker produces the utterance proportionally \red{(softmax?)} to the probability that the listener would infer the world $x$ given the utterance $u$: $P_{L}(x  \mid u)$, jointly with the prior probability of the utterance $P(u)$.
$$
P_{S}(u \mid x) \propto P_{L}(x  \mid u) \cdot P(u) \label{eq:RSA_S}
$$
...
Manipulating an utterance's cost can be used to derive Manner implicatures \red{(cite Bergen?)}, though in many applications utterance cost is kept uniform across alternative utterances. 
The specification of alternatives is an area of ongoing research \red{(cite something?, Franke cogsci 2015?, Peloquin and Frank cogsci 2016?)} and is currently determined by the modeler for the problem at hand.

The recursive speaker--listener model requires a base case in order to make predictions. 
RSA relies upon the notion of a literal semantics or a context-invariant mapping from utterances and worlds to truth values:  $\denote{u}: X \rightarrow \text{Boolean}$. 
From this, we can construct a base case of a \emph{literal listener}, who interprets utterances with respect to their literal meaning.
%
$$
P_{L_{literal}}(x \mid u ) \propto {\delta_{\denote{u}(x)} P(x)} \label{eq:RSA_L0}
$$
...
This recursion formalizes the Gricean maxim to be information: The speaker produces utterances not with respect to their literal meaning, but with respect to how a naive listener would interpret them, given the literal meaning and her prior beliefs.
This base case also captures the Gricean maxim to be truthful: the speaker who reasons about the literal listener has the goal of communicating the state $x$ given an utterance $u$ and its literal meaning $\denote{u}$. 
In principle, the speaker--listener recursion could grow for ever. In practice, stable pragmatic phenomena can be observed with just one level of recursion \red{(CITE something?)}. 

\mht{This paragraph is pretty abstract, and may be better suited for the discussion}
This general framework has been applied to host of language and reasoning phenomena including understanding scalar implicature \cite{Goodman2013, Degen2015}, communication in a noisy channel \cite{Bergen2015}, question--answer behavior \cite{Hawkins2015}, syllogistic reasoning \cite{Tessler2014}, and figurative language use like hyperbole \cite{Kao2014}, metaphor \cite{Kao2014b}, and irony \cite{Kao2015}.
In addition, RSA has been used to understand the very meaning of words in domains where meanings are hard to specify like vague adjectives \cite{Lassiter2015}.
These extensions often take the approach of introducing additional uncertainty for the listener. In addition to the listener being uncertain about the state of the world ($x$, above), she may have uncertainty about what is common ground, what is the question under discussion, and even what is the very meaning of the word. These kinds of uncertainty can be plausibly posited both at the pragmatic level ($L$) and/or at the base-case, the literal listener. 
However, empirical studies show that the richness of human pragmatic inference often requires this uncertainty only at the pragmatic level. 
This is because human reasoning about these kinds of uncertainty involve incorporating the Gricean pressure to be informative, which is only captured at levels of recursion beyond the base case.
...
We imagine a hypothetical, pragmatic listener ($L_1$) concerned with learning the prevalence of a certain property in a certain category, $x=P(F \mid K)$, who reasons about an informative speaker ($S_1$)
...
The listener also uses her intuitive theory of the speaker $S_{1}$ as an informed and helpful interlocutor to resolve the meaning.
\begin{eqnarray}
...
%We illustrate the model of generic interpretation using a number of 
...
\subsection*{Generics and conceptual structure}
