...
The fact that $\lambda_1$ is plausibly less than 1 suggests that the assumption that ``staying silent'' and producing the generic are equally costly (as currently assumed by the model) may not be correct.
However, we maintain using only the two $\lambda$ parameters in the model for simplicity.
...
Having corroborated the model with empirical data, we can again examine the model more closely.
The $\lambda_2$ parameter, which is only used to model the truth judgments task, is credibly below 1: participants' behavior is more in the direction of guessing than veridical Bayesian updating according to our model.
We assumed for the model's purposes that the prevalence that the speaker model was aiming to communicate was, in fact, the prevalence told to the participant.
However, if participants' beliefs about properties are inconsistent with the prevalence statement they are given (e.g., 90\% of lorches have broken legs), they may reject the stated prevalence, and assign a truth judgment to the generic based on theory-derived considerations.
If this is so, we would predict that the $\lambda_2$ parameter would be lowest for properties that are unlikely to be present at all prevalence levels (since the experiment was fully-randomized, each property type appears with each prevalence level).
This would be the case for the accidental properties (which are unlikely to be present in high prevalence) and the body part properties (which are unlikely to be present in low prevalence; e.g. 10\% of lorches have wings).
...
Indeed, we found that the $\lambda_2$ parameter is lowest for the rare accidental properties (HPD = [0.03, 0.24]), followed by the common accidental properties (MAP = [0.27, 0.60]) and then the body part properties (MAP = [0.33, 0.76]).
...
This suggests frequency evidence is interpreted through the lens of theoretical considerations, and that has important implications for generic truth conditions.
