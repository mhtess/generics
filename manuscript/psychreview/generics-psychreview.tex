...
%, though other forms can express it as well (e.g., \red{A moongoose eats snakes.} \cite{Carlson1977}).
...
Statistical and conceptual theories express contrasting views of the truth conditions of generic statements \cite{Carlson1995essay}. \footnote{We use the terms statistical and conceptual to refer to what \citeA{Carlson1995essay} referred to as ``inductive'' and ``rules and regulations'' views.}
...
Assuming the generic relates to the property prevalence, then the simplest meaning would similarly be a threshold on prevalence: $P(F\mid K)>\tau$. \citeA{Cohen1999} takes $\tau = 0.5$; that is, \emph{Robins lay eggs} is roughly taken to mean \emph{Robins, in general, lay eggs}. 
...
\subsection*{The pragmatic approach}
...
We pursue such a line of inquiry, assuming the simplest truth-conditional meaning: a threshold on prevalence $P(F\mid K)>\theta$ \cite{Cohen1999}.
...
Of course, no fixed value of the threshold, $\theta$, would allow for the extreme flexibility generics exhibit (e.g. \emph{Mosquitos carry malaria}; \emph{Robins lay eggs} v. \emph{Robins are female}).% simultaneously with the interpretations of some novel generics being strong. 
...
%
\begin{eqnarray}
L_{1}(x , \tau \mid u) &\propto& S_{1}(u \mid x, \tau) \cdot P(x) \cdot P(\tau) \label{eq:L1}
\end{eqnarray}
%
...
\begin{eqnarray}
S_{1}(u \mid x, \tau) &\propto& \exp{(\lambda_1 \cdot \ln  ( {L_{0}(x \mid u, \tau)} ) ) }\label{eq:S1}
\end{eqnarray}
%
...
The listener believes the speaker $S_1$ knows the threshold $\tau$.
...
The silent or \emph{null} utterance alternative carries no information (i.e.~$\denote{null}(x, \tau)=T$) and produces a posterior distribution in the listener identical with the prior.\footnote{
...
The literal listener has access to the threshold $\tau$ as well, and simply restricts her prior beliefs to situations where the truth-functional denotation of the utterance, $\denote{u}$, is true (for the silent utterance: all worlds; for the generic utterance: the worlds with prevalence greater than $\tau$).
%
\begin{eqnarray}
L_{0}(x \mid u, \tau) &\propto& {\delta_{\denote{u}(x, \tau)} P(x)} \label{eq:L0}
\end{eqnarray}
%
...
\begin{equation} 
P_{S_{2}}(u \mid x) \propto  \exp{(\lambda_2 \cdot \ln \int_{\tau} P_{L_{1}}(x , \tau \mid u) \diff \tau ) }% =  P_{L_{1}}(x \mid u)
\label{eq:S2}
\end{equation}
...
Critically, in contrast to the hypothetical speaker $S_1$, speaker $S_{2}$ doesn't actually know what the generic means (i.e. doesn't have access to the threshold $\tau$), but knows that it will be inferred by $L_{1}$, and integrates over the likely values she'll consider.
...
The same basic phenomenon can be observed for the other two example priors (top row): The posterior over prevalence heavily depends upon the prior. %the quantitative details of this inference are heavily dependent on the shape of the prior distribution (i.e. the posteriors look dramatically different).
...
From the prevalence-prior data (Expt.~1a), we estimate participants' beliefs about the prevalence of a property \emph{for a given kind} (e.g.~the percentage of \textsc{robins} that \textsc{lay eggs}; see green intervals on Figure \ref{fig:priors1a} and \emph{Appendix}).
...
We find a little over half of the variance in truth judgments data is explained this way ($r^2 = 0.599$; MSE=0.065; \emph{SI Figure 2}). 
...
As we see in Figure \ref{fig:modeldataBars}, the pragmatic speaker model $S_2$, using empirically measured priors, explains nearly all of the variance in human truth judgments ($r^2=0.98$; MSE=0.003; Figure \ref{fig:modeldataBars}). 
Generics that received definitive agreement or disagreement are predicte to be judged as such by the model (corners of Figure \ref{fig:modeldataBars}), including items for which target-category prevalence is not a good indicator of the acceptability (e.g. \emph{Mosquitos carry malaria}, for prevalence quartiles 2 and 3, $r_{Q2,3}^2=0.955$; MSE=0.005; Figure \ref{fig:modeldataBars}, intermediate shades}.
...
\subsection*{Extended model exploration}

Our probabilistic pragmatics model only has a prevalence-based semantics yet it is able to explain the flexibility in truth judgments for a diverse range of generic statements.
...
Having validated our model with empirical truth judgments, we can examine where in our model structured representations may play a role. 
...
For instance, if participants believe that some kinds have a causal mechanism that \emph{could} give rise to the property, while others do not, then we would expect $P(x)$ to be structured as a mixture distribution (cf. \citeA{Griffiths2005}).
We know that our priors lead the pragmatics model to the correct truth judgments. 
We now explore the hypothesis that these are priors are structured.
...
If a kind cannot, the prevalence can only be 0\%.\footnote{This other component (``the kind doesn't have a causal mechanism that would give rise to  the property'') could instead reflect accidental causes of the property, in which case, the prevalence could be non-zero. This an interesting possibility but full consideration is beyond the scope of this article.} 
...
Thus, $\phi$ is the \emph{potential of a property to be present in a kind} and $\gamma$ is the \emph{mean prevalence of the property among the kinds with the potential to have it}. \footnote{We note that $\phi$ is not what other authors have described as \emph{cue validity}, or $P(K \mid F)$. $\phi$ is a mixture component in the distribution over $P(F\mid K$)
}
The prevalences given by participants would then be distributed as: $P(d) = \phi \cdot \text{Beta}(d \mid \gamma,\xi)+ (1 - \phi) \cdot \delta_{d=0} $. 
...
$\gamma \sim \text{Uniform}(0,1)$, $\xi \sim \text{Uniform}(0, 50)$, and implemented the structural hypothesis in a Bayesian statistical model.
We performed Bayesian inference separately for each property using the Metropolis-Hastings algorithm to estimate the posterior distribution over the latent parameters $\phi, \gamma$, and $\xi$ with 3 MCMC chains of 100,000 iterations removing the first 50,000 iterations for burn-in.

\subsubsection*{Results}


The mixture parameter $\phi$ and the mean of the ``has the potential'' component $\gamma$ for each property are shown in Figure \ref{fig:priors1a}.
We see significant diversity among these properties in both dimensions, corresponding to priors over prevalence with dramatically different shapes (insets). 


Again, we look to the posterior predictive distribution to validate the structured prior model.
Using the model with it's inferred parameters, we generate prevalence judgments for different properties (assuming the structure) and compare that to the empirical counts. 
...
The other test of this hypothesis is to re-examine the truth judgments using the inferred structured priors (as opposed to the raw empirical counts). 
We find the same correspondence to the empirical truth judgments data ($r^2 = 0.98$), providing further evidence that the prior distribution over prevalence $P(x)$ is structured.
...
\section*{Empirical test 2: Strong interpretations for novel categories}
...
To answer this, we adopt an experimental paradigm used by \citeA{Gelman2007}.
...
In Experiment 3b, we aim replicate the basic finding of \citeA{Gelman2007}, and compare the truth judgments in this task to the model predicted endorsements.

\section*{Experiment 3a: Predictive prevalence elicitation}

The design of this experiment is based on a study reported in \citeA{Gelman2007}.
...
\subsubsection*{Procedure and materials}
Following \citeA{Gelman2007}, on each trial, participants read a vignette about a novel creature. For instance,
\begin{quote}
...
In this experiment, we test the predictions of our model by aiming to replicate \citeA{Gelman2007}.
...
Much of the extant empirical literature on generics argues that prevalence alone is insufficient to account for the truth conditions of generic statements.
The typical empirical demonstration is to measure felicity judgments of common generic statements (as we did in Expt.~1b) and ask a separate set of participants about the prevalence of the property for the target category \cite<e.g. the percentage of birds that lay eggs; >{Khemlani2012, Prasada2013}.
Our claim is not that the prevalence of the property for the target category alone explains the felicity judgments. 
Rather, we claim the target-category prevalence must be interpreted with respect the prior distribution on prevalence. 
...
The bare plural construction can also indicate a specific plural predication (consider \emph{dogs are on my lawn} compared to \emph{dogs have fur};  \citeNP{Carlson1977}).
...
For instance, \citeA{Declerck1991} suggests that generic and nongeneric bare plurals can be treated in the same way, and that pragmatic considerations may resolve interpretative differences. 
