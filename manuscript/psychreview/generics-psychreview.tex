...
Further, since prevalence is a predictive probability, our model can take information conveyed with a generic and extend that information to actual entities in the world. 
...
The utility function is based on the informativeness of the utterance. Informative is evaluated with respect to what a naive listener $L_0$ would believe upon hearing the utterance. 
...
The speaker's decision is made with respect to a hypothetical literal listener $L_0$. The literal listener evaluates an utterance according to its truth-functional denotation, a notion adopted from formal semantics. The semantics is a context-invariant mapping, from utterances and worlds to truth values:  $\denote{u}: X \rightarrow \text{Boolean}$. 
...
The RSA framework formalizes the Gricean maxim to be information: The speaker produces utterances not with respect to their literal meaning, but with respect to how a naive listener (the literal listener) would interpret them, given the literal meaning and the listener's prior beliefs.
