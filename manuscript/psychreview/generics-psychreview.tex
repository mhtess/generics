...
Assuming the generic relates to the property prevalence, then the simplest meaning would similarly be a threshold on prevalence: $P(F\mid K)>\theta$. \citeA{Cohen1999} takes $\theta = 0.5$; that is, \emph{Robins lay eggs} is roughly taken to mean \emph{Robins, in general, lay eggs}. 
...
\begin{equation} 
P_{S_{2}}(u \mid x) \propto  \exp{(\lambda_2 \cdot \ln \int_{\theta} P_{L_{1}}(x , \theta \mid u) \diff \theta ) }% =  P_{L_{1}}(x \mid u)
\label{eq:S2}
\end{equation}
