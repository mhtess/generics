...
\mht{you wrote: "this paragraph needs editing for flow.".... now?}
...
Generic statements (e.g. \emph{Glippets have yellow fur.}) thus show an asymmetry between their \emph{truth conditions} and the prevalence implied by the statement; this is in contrast to the behavior of quantified statements involving ``all'' or ``most'' \cite{Cimpian2010}. 
Upon reading a generic, participants infer (\emph{implied prevalence}) that the property is widespread (e.g. almost all glippets have yellow fur).
By contrast, participants endorse generics (\emph{truth conditions}) for a wider range of prevalence levels (e.g. even when ``30\% of glippets have yellow fur.''), thus showing an asymmetry between truth conditions and implied prevalence. 
However, this mismatch is significantly reduced for generics of accidental properties  (e.g. \emph{Glippets have wet fur.}).
...
Cue validity may be correlated with our model predictions, but it is distinct.
This correlation comes at least in part from the mixture parameter $\phi$ (the potential of the property to be present in a kind) used in the prior distribution on prevalence. 
The correlation isn't perfect, however: for the property \emph{eats people}, the mixture parameter $\phi$ is very low because most animals don't eat people; yet learning that some animal \emph{ate a person} is not a particularly good cue for any one kind of animal.
