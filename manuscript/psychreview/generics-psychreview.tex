...
\subsubsection*{Disclosures and Acknowledgments}

This research was supported in part by National Science Foundation Graduate Research Fellowship DGE-114747 (to M.H.T.),
...
Funding agencies had no other role other than financial support.

All authors contributed in a significant way to the manuscript.
...
\footnote{However, see \citeA{Cohen2004} for a discussion of how his semantic constraints relate to different kinds of generics and different kinds of conceptual representational frameworks found in cognitive science.}
...
%\mht{You said: "clean up this section to read less listy and more like a description of the essential ideas from this viewpoint, and highlights of empirical support (if that's relevant)."... How does this seem?}
...
\citeA{Leslie2007}'s influential theory posits that generics are tied to a ``default mode of generalization''. 
...
%\mht{anything more from Leslie?}

Parallel work in the psychology of concepts supports this perspective.
\citeA{Prasada2006} and later \citeA{Prasada2013} distinguish between \emph{principled}, \emph{statistical}, and \emph{causal} relations within concepts. 
Generics like \emph{Birds lay eggs} (in which only a minority of K have F) exhibit characteristics of principled connections (operationalized by endorsement of the phrase \emph{In general, Ks have F}), supporting \citeA{Leslie2007}'s typology.
...
%For example, these include the expectation that the relation will hold for future instances , the explanation that the relation holds because of the kind of thing that K is (\emph{Ks have F by virtue of being a K}), and that the principled relation is normative (\emph{Ks should have Fs}).
%These different kinds of relations relate to \citeA{Leslie2007}'s distinctions among generics.



%Such an argument needs to be further augmented to capture intuitions: non-malaria carrying mosquitos must \emph{have the potential} to carry malaria, while other insects or animals must not. Otherwise, \emph{Insects carry malaria} or \emph{Animals carry malaria} would also be true \cite{Leslie2007}.
...
We pursue such a line of inquiry, assuming the simplest truth-conditional meaning: a threshold on prevalence $P(F\mid K)>\theta$ \cite<cf.>{Cohen1999}.
...
RSA is derived from social reasoning, formalized as recursive Bayesian inference between speaker and listener (\citeNP{Frank2012, Goodman2013, Goodman2016}; see also, \citeNP{Franke2009, Franke2015}).
\citeA{Goodman2016} provides a good introduction to the RSA framework and Appendix A presents a brief tutorial on RSA for the reader unfamiliar.
...
For a full list of the properties, and generic sentences used in Expt.~1b, see Table 1 (Appendix).
...
They were asked to press one of two buttons to signify whether they agreed or disagreed with the sentence (see Table 1 in Appendix for complete list). 
...
From the prevalence prior data (Expt.~1a), we estimate participants' beliefs about the prevalence of a property \emph{for a given kind} (e.g.~the percentage of \textsc{robins} that \textsc{lay eggs}; see green intervals on Figure \ref{fig:commongenerics}a insets, and Table 1 in Appendix).
...
The Maximum A-Posteriori (MAP) estimate and 95\% Highest Probability Density (HPD) interval for $\lambda_1$ is 0.5 [0.004, 12.7] and $\lambda_2$ is 1.7 [1.3, 2.1]. %(full posterior distributions can be found in Appendix B; Figure \ref{fig:tj1-params}).
\footnote{
...
For instance, if participants believe that some kinds have a causal mechanism that \emph{could} give rise to the property, while kinds others do not, then we would expect $P(x)$ to be structured as a mixture distribution (cf., \citeNP{Griffiths2005}).
...
We note that $\phi$ is not what other authors have described as \emph{cue validity} \cite{Beach1964, Khemlani2012}, or $P(K \mid F)$. 
...
All materials are shown in Table 2 in the Appendix.
...
%Generic statements (e.g. \emph{Glippets have yellow fur.}) show an asymmetry between the prevalence required for them to be judged true and the prevalence inferred from an assertion of the statement; 
\citeA{Cimpian2010} found that upon reading a generic (e.g. \emph{Glippets have yellow fur.}), participants infer (in an \emph{implied prevalence} task) that the property is widespread (e.g. almost all glippets have yellow fur).
...
is significantly reduced for generics of accidental properties  (e.g. \emph{Glippets have wet fur.}). % \cite{Cimpian2010}. 
...
Below we replicate the basic asymmetry findings of \citeA{Cimpian2010} and reveal even more variability in the mismatch between \emph{truth conditions} and \emph{implied prevalence} using the expanded stimulus set from Expt.~2a.
...
%%% removed this footnote:
%We assume here that the prevalence told to the participant is the subjective probability that that speaker $S_2$ is trying to communicate. In some situations this assumption may be invalid due to theory-driven considerations on behalf of the speaker. For example, if the participant believes the prevalence being reported in the experiment is describing a temporary or accidental state and not one that is likely to be predictive of the future prevalence, the speaker may derive a subjective probability substantially less than that stated verbally in the experimental prompt.


%
%\subsection*{Extended model analysis}
%
%\mht{you wrote: "i don't think the overall logic of this paragraph, connecting $\lambda_2$ to theoretical mismatch, will be clear to readers. explain better?" ... now?}
%\ndg{not really... i think we should remove this subsection and just segue directly to the subjective probability....}
%Having corroborated the model with empirical data, we again examine the model more closely.
%The $\lambda_2$ parameter, which is only used in the speaker $S_2$ model of the truth judgment data, is a soft-max parameter linking the $S_2$ model's probability of generic endorsement to the human behavioral data. Values below 1 imply that participants' behavior is slightly more random than one would expect from veridical Bayesian updating according to our model. Indeed, the value of the $\lambda_2$ parameter was found to be credibly less than 1 (see above).
%This suggests that either the artificiality of the task leads participants to produce a substantial amount of random behavior or there is something in our model that isn't specified correctly.
%
%Recall that in order to generate $S_2$ model predictions above, we assumed that the prevalence level told to the participant in the experimental prompt was, in fact, the prevalence level that the speaker model $S_2$ seeks to communicate.
%That is, we have assumed here the semantic scale over which generic gets evaluated and interpreted is the \emph{objective}, current frequency in the world.
%If, however, what matters for generic language is a speaker's \emph{subjective} belief about the world (e.g., a prediction about the prevalence in the future), then we would expect the model's predictions (here, the truth judgments) using the objective information to deviate from the human data, when subjective and objective frequencies disagree the most.
%This deviation would be particularly pronounced for the objective information that is most inconsistent with participants' intuitive theories of properties.
%
%If the $\lambda_2$ parameter were inferred for each type of property independently, 
%we would expect it to be lowest for types of properties for which participants beliefs would be most inconsistent with all prevalence levels given to the participants (i.e., some prevalence levels are very implausible).
%% with the strongest \emph{a priori} beliefs about prevalence that are skewed to either low or high prevalence levels. 
%%These properties are that are unlikely to be present at \emph{all} prevalence levels given in the experiment. This is because, on average, the objective frequency would deviate the most from 
%This would be the case for the accidental properties (which are unlikely to be present in high prevalence; e.g. 90\% of glippets have broken legs) and the body part properties (which are unlikely to be present in low prevalence; e.g. 10\% of lorches have wings).
%To investigate this, we implemented a model where the $\lambda_2$ parameter was allowed to vary by property-type.
...
%Indeed, we found that the $\lambda_2$ parameter is lowest for the rare accidental properties (95\% HPD = [0.03, 0.24]), followed by the common accidental properties (HPD = [0.27, 0.60]) and then the body part properties (HPD = [0.33, 0.76]).
%For the colored and vaguely described body parts --- for which all levels of prevalence are plausible ---  the speaker optimality parameters were considerably higher (color HPD = [0.50, 0.99]; vague HPD = [0.72, 1.32]).
%This is consistent with the hypothesis that the underlying scale that the truth conditions of generic statements are evaluated against is not objective frequency but subjective probability (i.e., the probability that a future instance of the kind would have the feature).
%We directly test this hypothesis in the next experiment.
...
We see that, in our paradigm, participants are also sensitive to the outcome of the event.%This is surprising because \citeA{Gelman2007} observed no effect of the event outcome on generic endorsement.
...
This is because both color and body part priors are relatively broad (see schematic predictions from Figure \ref{fig:schematic-unif} ``have wings'' for comparison).
...
Our model makes two novel predictions for generic endorsement in the paradigm by \citeA{Gelman2007}.
...
%%NDG: i commented this out because it's weird and distracting. can put it back if reviewers suggest this is a confound....
%\footnote{
...
%In this discussion, we discuss the previous empirical findings that have been used to argue that property prevalence is not sufficient for formalizing generic language.
...
There have been numerous demonstrations arguing that statistics (e.g., property prevalence) are insufficient to explain generic meaning \cite{Gelman2002, Gelman2007, Cimpian2010, Cimpian2010c,Khemlani2012, Prasada2013}.
...
In these experiments, the prevalence considered is only the prevalence of the property for the target category \cite<e.g. the percentage of birds that lay eggs; >{Khemlani2012, Prasada2013}, what we have referred to as \emph{within-kind prevalence}. 
Indeed, this simple statistic also fails to explain our data (Figure \ref{fig:commongenerics}b, right).
Our formal model, by contrast, considers not only within-kind prevalence, but a listener's prior beliefs about prevalence across kinds in order to arrive at a meaning for a generic utterance.
...
%It has also been argued that \emph{cue validity} --- the probability of the kind given the feature: $P(K \mid F)$  --- is an important factor in generic meaning \cite<e.g., >{Khemlani2012}.
%For example, the \emph{cue validity} of malaria for mosquitos (i.e., the probability that an animal is a mosquito given that it carries malaria) is very high. 
%Cue validity is often correlated with our model predictions, supporting its explanatory value in previous studies, but it is distinct.
%The reason for preferring a semantics based on prevalence $P(F \mid K)$ alone is that it provides a way to go from generic statements to beliefs about individuals in the world. 


%It also unifies the treatment of generic statements and quantifier sentences, a result often believed to not be possible \cite{Leslie2012default, Prasada2013, Gelman2015d}.

%In our model the mixture parameter $\phi$ (the potential of the property to be present in a kind) used in the prior distribution on prevalence has a large influence on generic endorsement. The cue validity $P(K \mid F)$ will often be high when only a single category is be capable of having the property, hence $\phi$ will be low and the generic will tend to be good. But this is only one special case: the generic can be good when $phi$ is not small, and $phi$ can be small for without cue validity being high. For instance, for the property \emph{eats people}, the mixture parameter $\phi$ is very low because most animals don't eat people; yet learning that some animal \emph{ate a person} is not a particularly good cue for any one kind of animal.
%\ndg{i revised this paragraph, but i'm not sure that it's right.... the connection between phi and cue validity doesn't really go either way. instead, i think cue validity is high when the category is in the right tail of the prevalence distribution or something? we should either work through the relation more carefully, or say something vaguer that doesn't hinge on phi....}

%\ndg{argument gets a bit fuzzy here... want to say that cue validity may be correlated with our model but is distinct. correlation comes at least partly from $\phi$?}
%In our formal model,
%However, cue validity is defined with respect to a particular kind and property, whereas our mixture parameter is a function of the property alone. 
%That is, our theory makes the claim that speakers need not believe that the cue validity for a kind and property is high, only that the potential for the property to be present is low.
%For instance, 

%We note that unlike a data-analytic model (e.g., a regression model) that uses predictors of prevalence and cue-validity to \emph{describe} data \ndg{cite?}, our model is a unified cognitive model --- a formal instantiation of an explicit psychological hypothesis --- that \emph{predicts} both generic endorsements and interpretations from a simple set of assumptions (a prior belief distribution and general pragmatic computation). \ndg{this paragraph seems a little heavy-handed. cut it?}
...
%Cue validity is defined as %By Bayes' rule, cue validity must take into account the prevalence of the property in the kind and the prior probability of that kind $P(K \mid F) \propto P(F \mid K) \cdot P(K)$. 
...
\citeA{Nickel2008} argues that \emph{Dobermans have floppy ears} may be true in the context of a discussion of evolutionary biology but that \emph{Dobermans have pointy ears} is true in a discussion of dog breeding.
...
%Our theory, thus, makes the prediction that generics should be subject to context-effects in the a way analogous to how gradable adjectives (e.g., \emph{tall}) are subject to context-effects.
...
%The comparison class also points to an additional mechanism by which the slipperiness of generics may manifest; if a speaker and listener fail to coordinate on the same comparison class, our theory predicts disagreements into the veracity and interpretation of generic claims.


%This sort of context matters for our model as well. 
%This framework opens up a number of interesting directions. 
%This is the first work to show how the understanding of generic language can depend on context.
%Our model for understanding generic language relies upon interlocutors' belief distribution over the statistics of the property to accurately arrive at generic interpretation identical to that of human participants. 
%The prior belief distribution for a property (e.g., lays eggs), is a distribution over kinds (and their associated prevalence of the property).
%These other kinds form a \emph{comparison class} against which the target kind is evaluated. 
%$P(x)$ is always relative to a category of categories, the comparison class: $P_K(x)$. . This would be our approach to understanding the context effects described by \citeauthor{Nickel2008} and \citeauthor{Sterken2015}.
...
%\ndg{need to expand a bit on this? maybe say that the prior $P(\theta)$ really needs to be considered as relative to a category of categories (the comparison class) $P_K(\theta)$?}

%\ndg{maybe put past and recent work on comparison classes first in this section and then point out that this matters in our model as well? also don't make it seem like we have a solution to this problem, just a place to start on it in future work.}



%As we move away from statements about animals, determining what goes into the contrast class becomes a lot more complicated. 

%\ndg{say that throughout our experiments we used animal target kinds, which lead naturally to comparison class of other animals. future work to extend to other kinds; sometimes the comparison class is less clear....}





%\mht{discuss more the relationship to gradable adjectives. that our theory is the first to posit such a thing for generics, that we unify the treatment of adjectives and generics?}
...
%And though prevalence may not be considered consciously by the interlocutor when thinking about generics, the theory presented here suggests that this statistical information is along for the ride. 
...
%This resolves another theoretical issue in the generics literature: Current frequency is not sufficient \ndg{need cite}. %\footnote{This is usually paired alongside ``Frequency is not necessary'' and given the example of ``Mosquitos carry  malaria''.} 
...
Thus, we would still reject the generic \emph{Supreme Court Justices have even social security numbers}, because the predictive prevalence would not be any different for Supreme Court Justices than any other profession.\footnote{Our perspective makes the intriguing prediction that if we learned much more surprising information, we might be compelled to revise our theory and thence accept the generic. For instance, if all justices had \emph{prime numbered} social security numbers (a more suspicious coincidence), one might appeal to a conspiracy, which \emph{would} have predictive consequences.}
...
%Of particular importance may be beliefs about which types of properties are most \emph{projectible}  into the future \cite{Goodman1955}. 


%Such a representation would give rise to the prevalence distributions we observed are foundational for understanding generic language. 
...
%Our theory claims that causal beliefs about kinds and properties lead to differences in prevalence distributions, and these differences lead to variable endorsements and interpretations of generic sentences. 
%In our experiments with novel generics, we showed how the structure of prevalence priors about different kinds of properties leads to variable inferences. 
%Biological properties tend to have bimodal prior distributions on prevalence (with 0\% and 100\% prevalence being the most likely \emph{a priori}). Such beliefs are updated, upon hearing a generic, to concave posterior distributions peaked at 100\%.
%In contrast, accidental properties have unimodal prior distributions (peaked at only 0\%), which support weaker generic interpretations. 
...
%These predictions were corroborated by the empirical data of generic interpretation. 
...
%In addition to differences in the comparison class, our theory posits two other ways 
%individual differences in generic endorsement and understanding may arise: 
...
\section*{Appendix A: The Rational Speech Act framework}

The goal of this section is introduce the reader to the Rational Speech Act (RSA) framework. \citeA{Goodman2016} provides another introduction and useful review of the framework.

The Rational Speech Act theory views language understanding as a special case of social cognition instantiated in a Bayesian model (\citeNP{Frank2012, Goodman2013, Goodman2016}; see also, \citeNP{Franke2009, Franke2015}).
In this view, an utterance is \emph{interpreted} by a Bayesian listener by integrating her prior beliefs with the likelihood that a speaker would have generated the utterance.
This generative model of the utterance is the listener's intuitive theory of speech \emph{production}, i.e. her theory of a speaker. 
This intuitive theory of speech production views speakers as rational actors who behave in accord with Bayesian decision-theory \cite{berger1988}.
The rational speaker, thus, chooses utterances soft-max optimally on the basis of  conveying information to a naive listener. 
The pragmatic listener, then, ``inverts'' her model of a speaker to arrive at a pragmatic interpretation of an utterance.
...
In simplest form, a pragmatic listener ($L_1$) is interested in learning about the world $x$ upon hearing an utterance $u$. 
$$
L_1(x  \mid u) \propto S_1(u \mid x) \cdot P(x) \label{eq:RSA_L}
$$
She does this by integrating her prior beliefs about the world $P(x)$ with the likelihood that a speaker ($S_1$) would have produced the utterance $u$ given a world $x$: $S(u \mid x)$.
Prior beliefs about the world $P(x)$ capture relevant world-knowledge, which is central to deriving context-sensitive interpretations.

The utterance production model $S_1$ is a rational (speech-)actor model. According to Bayesian decision theory, the speaker takes actions (i.e., produces utterances) soft-max optimally in accord with his utility function \cite{Baker2009}\footnote{See \url{agentmodels.org} for more on this form of model specification.}.
The utility function of a speaker is based on the informativeness of the utterance. Informativeness is computed with respect to what a naive listener $L_0$ would believe upon hearing the utterance. 
...
$$U_{S_{1}}(u; s) = \ln (L_{0}(s\mid u)) - C(u)$$
The speaker is then a soft-max optimal agent, with degree of optimality governed by $\lambda$.
$$
S_{1}(u \mid x) \propto \exp{(\lambda \cdot U_{S_{1}}(u; s) )}\label{eq:RSA_S}
$$
...
The speaker's decision is made with respect to a hypothetical literal listener $L_0$. The literal listener evaluates an utterance according to what the words mean: their truth-functional denotation, or semantics. The semantics is a context-invariant mapping, from utterances and worlds to truth values:  $\denote{u}: X \rightarrow \text{Boolean}$. 
...
%
$$
L_{0}(x \mid u ) \propto {\delta_{\denote{u}(x)} \cdot P(x)} \label{eq:RSA_L0}
$$
Note that this model is the same as $L_1$ except for the likelihood function. 
%
...
The RSA framework formalizes the Gricean maxim to be informative: The speaker produces utterances not with respect to their literal meaning, but with respect to how a naive listener (the literal listener) would interpret them, given the literal meaning and the listener's prior beliefs.
The fact that the literal listener updates her beliefs based on the truth-functional meaning of the utterance results in the speaker behaving in accord with the Gricean maxim to be truthful: the speaker who reasons about the literal listener has the goal of communicating the state $x$ given an utterance $u$ and given its literal meaning $\denote{u}$. 
In principle, the speaker--listener recursion could grow indefinitely. 
In practice, stable pragmatic phenomena can be observed with just one level of recursion.
...
%These applications often extend RSA by introducing an additional uncertainty for the pragmatic listener. 
%For example, in addition to the listener being uncertain about the state of the world ($x$, above), she may have uncertainty about what is in common ground, what is the topic being discussed (the Question Under Discussion, or QUD), and even what is the very meaning of the word. 
%This kind of uncertainty posited at the level of the pragmatic listener can be resolved incorporating the Gricean pressure to be informative.
...
Maximum A-Posteriori (MAP) estimates and 95\% Highest Probability Density Intervals are shown for each measure.
