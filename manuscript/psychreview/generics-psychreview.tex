...
\ndg{figure uses condition names `extrinsic/intrinsic' but those aren't used/introduced in text....}
...
These differences may account for the differences in results. \ndg{this footnote is very cryptic because you haven't said what the original results were...!}
}
...
In our formal model, the mixture parameter $\phi$ (the potential of the property to be present in a kind) used in the prior distribution on prevalence is conceptually similar. 
...
For instance, for the property \emph{eats people}, the mixture parameter $\phi$ is very low because most animals don't eat people; yet learning that some animal \emph{ate a person} is not a particularly good cue for any one kind of animal.
...
It makes the prediction that generics should be subject to context-effects in the a way analogous to how gradable adjectives (e.g., \emph{tall}) are subject to context-effects.
...
Recent work in philosophy and linguistics suggest that such context-effects are present for generics \cite{Nickel2008, Sterken2015}.
...
Our theory provides an explicit mechanism through which this context sensitivity would manifest: the comparison class.
...
There are some hints that the comparison class can be derived with respect to the property \cite{Keil1979}.
...
The incorporation of a comparison class into the generic language understanding points to an additional mechanism by which the slipperiness of generics may manifest; if a speaker and listener fail to coordinate on the same comparison class, our theory predicts disagreements into the veracity and interpretation of generic claims.
...
Indeed, we elucidated in Expt.~3, that participants' \emph{predictions} of probability perfectly track generic endorsement, when the present frequency would not correctly predict generic endorsement.
