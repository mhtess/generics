...
\subsubsection*{The pragmatic approach}

We find both the statistical and the conceptual accounts compelling, but feel there is a third perspective that is underrepresented.
...
The pragmatic lens reveal that utterances carry a mosaic of interpretations with a complex sensitivity to context \cite{Clark1996,Grice1975,Levinson2000}. 
...
Abstract mental representations, then, may be the backdrop against which generic language gets interpreted. 

We follow \citeA{Cohen1999} and use a threshold semantics that is judged against prevalence: $P(F\mid K)>\tau$.
...
Of course, no fixed value of the threshold, $\tau$, allows for the extreme flexibility generics exhibit (e.g. \emph{Mosquitos carry malaria}; \emph{Robins lay eggs} v. \emph{Robins are female}).% simultaneously with the interpretations of some novel generics being strong. 
...
This inference depends on background knowledge about properties and categories---potentially, structured conceptual knowledge---but is otherwise a general mechanism of language not specific to interpreting generic statements.
We formalize this hypothesis in the Rational Speech-Acts (RSA) theory---a probabilistic model of language understanding---as recursive Bayesian inference between speaker and listener \cite{Frank2012,Goodman2013}.
The reader unfamiliar with RSA is referred to Appendix A for a brief tutorial.
...
\end{figure}
    ...
    Endorsement probabilities for the generic as predicted by the $S_2$ model increase as a function of prevalence, but what counts as ``true'' depends on the prior. (Horizontal dashed line at 0.5 is for convenience of comparing the point at which an utterance is neither true nor false.)
...
This model makes the prediction that the interpretation of a generic statement depends on the prior on prevalence $P(x)$.
We explore these predictions by running simulations of $L_1$ under various schematic priors.
In Figure \ref{fig:schematic-unif}, are $L_1$ (Eq.~\ref{eq:L1}) marginal posterior distributions over prevalence $x$ (red solid line) for a couple example prior distributions over prevalence $P(x)$ (blue dashed line).
We give the example prior distributions names of properties that might be associated with such priors.
First we explore generic interpretation for priors of three different shapes (top row). 
For each of these, the posterior distribution over prevalence is heavily driven by the prior, but is only slightly shifted to the right. 

When the \emph{a priori} prevalence is centered around 0.5 (top-left facet; this could correspond to the distribution over the property ``are female''),
% the posterior distribution over the threshold is centered around a value less than the mean of the prevalence prior distribution (dark blue line). 
