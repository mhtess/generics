...
The goal of this section is introduce the reader to the Rational Speech-Acts framework and highlight relevant applications when appropriately.
...
The production model $P_{S}$ is defined recursively with the listener model: The speaker produces the utterance proportionally to the probability that the listener would infer the world $x$ given the utterance $u$: $P_{L}(x  \mid u)$, jointly with the prior probability of the utterance $P(u)$.
$$
...
Manipulating an utterance's cost can be used to derive Manner implicatures \red{(cite Bergen?)}, though in many applications utterance cost is kept constant. 
...
This recursive model needs a base case in order to make predictions. RSA relies upon the notion of a literal semantics or a context-invariant mapping from utterances and worlds to truth values:  $\denote{u}: X \rightarrow \text{Boolean}$. From this, we can construct a base case of a \emph{literal listener}, who interprets utterances with respect to their literal meaning.
%
$$
...
%
Here, $\delta_{\denote{u}(x)}$ is the Kronecker delta function returning $1$ for states $x$ compatible with utterance $u$, and $0$ otherwise.
This recursion formalizes the Gricean maxim to be information: The speaker produces utterances not with respect to their literal meaning, but with respect to how a naive listener would interpret them, given her prior beliefs and the literal meaning.
