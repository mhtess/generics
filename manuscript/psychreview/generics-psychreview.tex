...
\end{quote}
%
For \citeauthor{Carlson1977}, people have different \emph{strategies} to verify if a kind has a particular type of property 
(e.g. the strategy for verifying if a kind \emph{lays eggs} is to consult only the female members of the kind). 
This theoretical analysis anticipated later accounts that appeal to structured, conceptual representations (e.g. \emph{Bishops move diagonally} not because most bishops do but because the laws of chess mandate that they do). 
These contrast with semantic theories of generics that appeal to the statistics of the world (e.g. \emph{Barns are red} because most barns are; \citeNP{Carlson1995essay}).
...
Generics like \emph{Birds lay eggs} (in which only a minority of K have F) exhibit characteristics of principled connections. 
Striking generics (e.g. \emph{Mosquitos carry malaria}) do not have the characteristics of principled connections, but do show characteristics of causal connections (operationalized using the phrase \emph{There is something about Ks that cause them to F}). 
Different kinds of kind--property relations exhibit qualitatively different kinds of connections, thus supporting Leslie's claims of the different types of default generalizations.
...
That is, the only individuals under consideration are the female members of kinds.
\emph{Birds lay eggs} is roughly taken to mean \emph{Most female birds lay eggs}. 
...
In addition, it is unclear how this semantic theory relates to the unspecified pragmatic theory and more generally, to the conceptual accounts of generics (however, see \citeNP{Cohen2004} for a discussion of how his semantic constraints relate to different kinds of generics and different kinds of conceptual representational frameworks).
...
Prior beliefs about the world $P(x)$ capture relevant world-knowledge, which is central to deriving context-specific interpretations \red{(cite Degen, Tessler, Goodman cogsci?)}.
...
This recursive Bayesian model formalizes the Gricean maxim to be information: The speaker produces utterances not with respect to their literal meaning, but with respect to how a naive listener would interpret them, given the literal meaning and the listener's prior beliefs (more precisely, what the speaker believes is the listener's prior beliefs).
...
In addition, RSA has been used to bridge the semantics--pragmatics divide, by using pragmatic reasoning to understand the very meaning of words in domains where literal meanings are hard to specify \cite<e.g. vague adjectives, >{Lassiter2015}.
These applications often extend RSA by introducing an additional random variable for the pragmatic listener. 
For example, in addition to the listener being uncertain about the state of the world ($x$, above), she may have uncertainty about what is common ground, what is the question under discussion \red{(cite Roberts?)}, and even what is the very meaning of the word. These kinds of uncertainty can plausibly be posited both at the pragmatic level ($L$) and/or at the base-case, the literal listener. 
