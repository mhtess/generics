...
The posterior predictive distribution compared to human judgments is shown in Figure \ref{fig:byItemScatter} (Left). 
...
We additionally plot the human judgments (after hearing the novel generic) with the mean conditional prevalence (before hearing the generic) in Figure \ref{fig:byItemScatter} (Right). 
Though both the posterior predictive distribution of the listener model (Eq.~1) given the generic utterance and the \emph{a priori} mean conditional prevalence explain roughly the same amount of variance in generic interpretation ($r^2 = 0.92, 0.93$, respectively), the \emph{a priori} mean conditional prevalence consistently \emph{underestimates} the interpreted prevalence.
