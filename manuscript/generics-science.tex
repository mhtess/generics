...
%Imagine talking with a 3-year-old about oxygen. 
%Oxygen is difficult to explain because it is so rarely observable and the function of oxygen is abstract. 
%You might say something generic like, ``Animals breathe oxygen to live.''. 
%Few would argue with the truth of this sentence, yet its meaning is difficult to specify. 

Imagine discussing the importance of wearing shoes with a 3-year-old. 
The toddler is making a good case why she shouldn't have to wear shoes if she doesn't want to. 
You counter with something generic like, ``Shoes have arch support, which is good for staying on your feet.''. 
Indeed, these shoes do; many shoes do.
Few would argue with the truth of this sentence, yet its precise meaning is difficult to specify. 

%Imagine discussing the importance of taking vitamins with a 3-year-old. 
%Vitamins are difficult to describe because they the category is so variable.
%You offer something generic like, ``Vitamins are have beneficial qualities.'. 
%Few would argue with the truth of this sentence, yet its meaning is difficult to specify. 

%other openers:
% "Vitamins make you strong."
% Why do I have to wear shoes? "Shoes have arch support."
...
They are thought to be essential to the growth of conceptual knowledge \cite{Gelman2004}. 
Despite their centrality to cognitive development and everyday conversation, the semantics of generic language is not well characterized. 
% stable meanings for generic sentences are hard to come by.
...
%For example, ``\emph{Some} animals breathe oxygen.'' is true because there are animals that do. 
...
At first glance, generics seem like universally-quantified statements (i.e. ``All shoes have arch support''), but generics are resilient to counter-examples like \emph{Vibrams}. 
...
``Some shoes have arch support.'' does not carry the same generalizability.
...
The Maximum A-Posteriori (MAP) estimates and 95\% Bayesian Highest Density Intervals (HDI) over the mean prevalence $\gamma$ for each property--category pairing of interest can be seen in Table \ref{tab:expt1}. 
...
From the 300 free responses for the animal production part of the task, 105 distinct animal names were extracted. 
The top 5 ranking animal names produced were Dog, Cat, Elephant, Giraffe, and Bear; these responses accounted for 20.3\% percent of the total produced animal names.
...
\centering
    ...
    Dotted lines show density by removing the mass at zero. (For display purposes, this is omitted for ``have wings'', for which the density is off the scale.)
...
By contrast, ``has manes'' is rare across categories, and has an expected value of around 50\% within-categories, though there is considerable uncertainty in participants' judgments as to what percentage of an arbitrary animal kind to expect to have manes (Figure \ref{fig:priors1a} bottom left).
...
We will use these recovered prevalence priors in addition to the prevalence estimates of category--property pairs later in the generic language model to predict the acceptability of generics of natural cases.
...
As a manipulation check, participants were asked at the end of th ge trials which button corresponded to ``Agree''.
...
%The estimated posterior density of the speaker rationality parameter is shown in Figure \ref{fig:rationality}. 
...
However, this puzzle is resolved when a speaker understands the generic to be vague and takes into account the distribution of the property in question. 
The second puzzle in the psychology of generic language surrounds how listeners interpret generic utterances.
...
Thus, the implications of generic statements go far beyond the evidence needed to accept them as valid utterances.

Not all statements in bare plural form carry these implications, however. 
Indeed, interpretations of these sentences are sensitive to listeners' beliefs about the properties in question. 
