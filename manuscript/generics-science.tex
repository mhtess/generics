...
Interpretations of generic language are readily explored in our model of a listener (Eq.~\ref{eq:L1}) who hears a generic and is trying to infer how widespread is the property. 
We follow \citeA{Cimpian2010} by using novel animal categories and properties (e.g. \emph{Lorches} and \emph{have green feathers}) to explore how generic language updates one's beliefs. 
...
Classic work in generalization suggests that beliefs about the prevalence of properties are different for different types of properties, including differences between properties that are conceivably biological \cite{Nisbett1983}. 
...
Like the behavioral data, the listener model (Eq.~\ref{eq:L1}) produced a gradient of interpretations contingent on the type of property in question and its corresponding \emph{a priori} mean within-kind prevalence (Figure \ref{fig:exp2b}, Right, solid line). 
These inferences align very well with participants endorsements and interpretations of novel generic utterances ($r^2(10) = 0.923$). 
