...
Generics can in some instances be felicitous based on a small amount of evidence while other times require a lot of evidence to be true. 
...
Generic language is also ubiquitous in everyday conversation as well as in child-directed speech \cite{Gelman2008}, and children as young as two or three understand that generics refer to categories and support generalization \cite{Cimpian2008}.
...
One major hurdle to formalizing generic language arises when one tries to describe the conditions by which a generic is true.
At first glance, generics seem like universally-quantified statements (i.e. \emph{All apples are red}), but unlike universals, generics are resilient to counter-examples (e.g. \emph{Granny Smith apples}). 
...
A second important puzzle surrounds how generics are interpreted. 
...
In this paper we offer a mathematical model of generic language in terms of pragmatic inference of the degree of evidence required to assert a generic. 
...
Semantic accounts of generics are usually given by appealing to either the statistics of the world (e.g. \emph{Barns are red} because most barns are) or to structured, conceptual representations (e.g. \emph{Bishops move diagonally} not because most bishops do but because this is a structural relations in the world (of chess) dictating they do) \cite{Carlson1995essay}. 
...
Understanding language depends on assumptions interlocutors make about each other, which can lead to rich interpreted meanings in context \cite{Clark1996, Grice1975, Levinson2000}. 
If there is uncertainty about what a generic means, then perhaps the puzzles of generic language can be understood as the effects of pragmatic reasoning about a statistical meaning---which doesn't require a special, mental relationship---that is underspecified in the language.
We explore this possibility using a recently developed formalism in probabilisitic models of language understanding \cite{Frank2012, Goodman2013, Franke2009}, the Rational Speech Acts (RSA) theory. 
...
This \emph{a priori} truth-judgment of generics was a significant predictor of the empirical truth judgments: true generics were significantly more likely to be agreed with than the indeterminate generics ($\beta = 3.14; SE = 0.15; z = -20.9$), as revealed by a mixed-effect logistic regression with random by-participant effects of intercept.
...
\centering
    ...
    \caption{Between- and within-category prevalence empirically elicited for twenty-one animal properties with five example prevalence priors and model-predicted posterior distribuitons.
    Properties cover a range of prevalence levels, both within- and between- categories.
    Pop-out plots display example prior distributions over prevalence, which are mixtures of the between- and within- category information (see main text for details). 
...
\centering
    ...
    Pop-out plots show prior distributions over prevalence (which includes both between- and within- category information) and posterior distributions after hearing the generic that corresponds to that type of property. }
  \label{fig:prior2}
\end{figure}
...
We explored a minimal extension of a general-purpose probabilistic model of language understanding wherein social-cognitive mechanisms act on uncertainty in the meaning of a generic utterance to produce a less uncertain meaning.
This less uncertain meaning is a blend of the listener's beliefs about the property in question and the communicative force of a speech-act. 
The system of equations can model both the role of the speaker to produce felicitous generic utterances and the role of the listener to interpret those utterances in context, with high quantitative accuracy.
...
The dark side of this is that herein lies one potential avenue for miscommunication or deceit: A speaker might assert a generic utterance that he himself would never accept, but get away with it if the listener's beliefs are sufficiently na\"{i}ve.  
This listener would then have a belief distribution even further from the truth. 
The other side of this coin is that generics are a powerful way to communicate and learn generalizations about categories, which could otherwise be difficult or costly to learn through direct evidence.
...
Much psychological and philosophical work on generics has looked beyond prevalence and focused on conceptual distinctions and relations \cite{Prasada2013, Leslie2007, Leslie2008}. 
...
\citeauthor{Leslie2007} suggests \emph{reproduction} is a characteristic property of animals and information that is striking (e.g. ``Tigers eat people.'') is useful and thus permitted to be a generic.
Where in the prevalence-based semantics could such conceptual distinctions come into play?
Beliefs about prevalence are represented as probability distributions in a framework that is useful for representing rich, structured knowledge of the world \cite{Goodmanconcepts}. 
It's plausible that the prevalence distributions focused on in this work are derived from richer conceptual knowledge, including higher-order conceptual knowledge about the nature of different properties \cite{Gelman2005, Keil1992}. 
...
It is inherently a process of fumbling in the dark, riddled with uncertainty. 
Cooperation allows interlocutors to align their uncertainty and ensure knowledge, in fact, be transmitted across space.
