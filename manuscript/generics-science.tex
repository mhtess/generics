...
\subsubsection*{The semantics and pragmatics of generic language}
...
For a given kind, $K$ (e.g.~\textsc{robins}), and a property, $F$ (e.g.~\textsc{lays eggs}), we refer to the probability that an object of kind $K$ has property $F$, that is $P(F\mid K)$, as the \emph{prevalence} of $F$ within $K$.\footnote{Because we aim to explain the psycholinguistics of generics, we are generally interested in the subjective probability, not the actual frequency in the world.}
...
$L_1$ knows that the meaning of the generic utterance, if used, depends on a prevalence threshold, $\theta$, but does not know what this threshold is: $\theta \sim \text{Uniform([0,1])}$.
...
Assuming that \emph{when the property is possible}, its prevalence is distributed according to a Beta distribution with mean $\gamma$ and concentration $\tau$: 
$P(x)=\theta \cdot \delta_{x=0} + (1-\theta) \cdot \beta(\gamma,\tau)(x)$.\footnote{This is similar in spirit to Hurdle Models of epidemiological data where the observed counts of zeros is greater than one would expect from standard models of count data such as the Poisson model \cite<e.g. adverse events to vaccines; >{hurdleModels}.}
Thus, $\theta$ is the potential of a property to be present in a kind and $\gamma$ is the mean prevalence of the property among the kinds with the potential to have it. %Intuition suggests that, for a given property (e.g. \textsc{lay eggs}), kinds fall into two distinct categories: those for which the property is possible (e.g. \textsc{ducks}) and those for which the property is impossible (e.g. \textsc{lions}).
...
This statistical model reproduced the prior elicitation data with a low reconstruction error ($r^2 = 0.94$; see Supplement Section A3), strongly supporting the assumption of a structured prior. 
%To evaluate this model of the prior against our data, we inferred credible values for $\theta$, $\gamma$, and $\tau$ for each property using Bayesian statistical techniques (see Supplement Section A3).
%The best inferred parameters result in predicted prevalence distributions that match the elicited priors well ($r^2 = 0.94$).
...
Figure \ref{fig:priors1a} shows the estimated mixture-parameter $\theta$ (the property potential to be present in a kind) and the mean prevalence when the property is present, $\gamma$. 
...
For example, increasing $\theta$ increases the relative probability mass at 0\% and works to relax the truth conditions by making a lower threshold more informative; increasing $\gamma$ will tend to make truth conditions stricter, by reducing the range of prevalence values that are higher than the prior expectation. 
...
A little over half of the variance in truth judgments data is explained by the prevalence of the property within the kind alone ($r^2 = 0.599$; MSE=0.065; Supplement Figure 1). 
...
\subsubsection*{Interpretations of novel generics} 
...
We compared the interpretations of the pragmatic listener in Eq.~\ref{eq:L1} to human judgments ($n=40$) about the likely prevalence of the property after hearing a generic about an unfamiliar kind (e.g. \emph{Lorches have green feathers.}). %, in the same spirit as \citeA{Gelman2002}. 
Human prevalence judgments after reading the generic were affected by the type of property and its corresponding \emph{a priori} mean prevalence when present $\gamma$: The more prevalent a property is expected to be \emph{a priori}, the stronger the implications of a generic statement ($\beta = 0.57; SE = 0.08; t(39) = 7.12; p < 0.001$)\footnote{These statistics are the result of a mixed-effects linear regression with a maximal mixed-effect structure: Random by-participant effects of intercept and slope}. 
...
For both behavioral data and model predictions (Eq.~\ref{eq:S2})  we computed the average prevalence that led to an assenting judgment, for each property type and participant, following the procedure used by \citeA{Cimpian2010} (see Supplement Section D for more details).
...
By leveraging this common ground, generics provide a powerful way to communicate and learn generalizations about categories, which would be difficult or costly to learn through direct experience.
