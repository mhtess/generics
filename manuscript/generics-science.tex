...
%
\citeA{Lassiter2013,Lassiter2015} have extended RSA to explain vague adjectives (e.g.~\emph{tall}) in terms of an underspecified threshold criterion (Cf.~\citeA{Qing2014}). 
...
\subsubsection*{The structure of prevalence priors} 
 ...
 $P(x)$ (in Eqs.~\ref{eq:L1}, \ref{eq:L0}) describes the belief distribution on the prevalence of a given property (e.g. \textsc{lay eggs}) across relevant categories. 
 This distribution is common-sense background knowledge and is conceivably very different for different types of properties. 
 We measure it empirically by having participants ($n=57$) report their beliefs about the prevalence of the target properties (e.g. \textsc{lay eggs, carry malaria}; 21 in total) for many different animal kinds, both pre-specified by the experiment (e.g. \textsc{robins, mosquitos}) and self-generated by the participant (see Supplement Section A2 for complete details)\footnote{Our experiments stay within the animal kingdom because we expect there to be considerably less variability in participants beliefs about animals than about other types of categories (e.g. social categories)}. 

For a given property (e.g. \textsc{lay eggs}), there are animals kinds for which the property is present (e.g. \textsc{ducks}) and animals for which the property is absent (e.g. \textsc{lions}).
...
We infer credible values for $\theta$, $\gamma$, and $\delta$ for each property using Bayesian statistical techniques (see Supplement Section A3).
The best inferred parameters predict prevalence distributions that match the prior elicitation data well ($r^2 = 0.95$).
This means the reconstruction error between data and model is low, and that the assumption of a structured prevalence prior is warranted. 
...
These shapes are the results of structured beliefs about prevalence: Figure \ref{fig:priors1a} shows the mixture-parameter $\theta$ and the mean prevalence from the distribution of kinds for which the property is present---$\gamma=\mathbb{E}[P_{present}(x)]$---for all 21 properties of interest. 
...
We tested the degree to which Eq.~\ref{eq:S2} coupled with the empirically-elicited $P(x)$ predicted that a given category--property pair (e.g. \textsc{robins} and \textsc{lay eggs}) would result in a felicitous generic (e.g. \emph{Robins lay eggs.}). 
...
From the prevalence-prior data, we can also estimate participants' beliefs about the prevalence of a property for a given kind (e.g.~the percentage of \textsc{robins} that \textsc{lay eggs}; see Supplement (Table 2) for Maximum A-Posteriori (MAP) estimates and 95\% Bayesian Highest Density Intervals (HDI) over the mean prevalence for each property--category pairing). %We first compare the predictions of Eq.~\ref{eq:S2} to 
...
\ndg{i think we need to focus this section, particularly the results, much more clearly on implied prevalence. there are two kinds of results given: showing that implied prevalence depends on property in a way predicted by priors (through listener model), and comparing this prevalence to baselines: truth conditions and prevalence that would be implied if generic had weakest meaning ("some"). (Alternatively it could be focussed on novel generics, so that doing the truth judgement task is more natural.)}
...
How is generic language interpreted so strongly if the criterion for endorsement is so flexible? 

A hint lies in the vagueness of generics: With an uncertain meaning and the pressure to be informative, interpretation is going to be heavily driven by \emph{a priori} beliefs about property. 
...
Biological properties are expected \emph{a piori} to be more prevalent within a kind than accidental properties, with fine-grained differences even among types of biological and accidental properties (Figure \ref{fig:prior2}; x-dimension).
...
%after necessarily ruling out the 0\% possibility upon hearing \emph{Ks have F} 
...
\citeA{Cimpian2010} reported a surprising d\'{e}colage between the interpretations of novel generics and the average prevalence required to assent to the exact same utterance.
We also recruited participants ($n=40$) to help determine the average prevalence at which a speaker would assent to the generic. 
%\ndg{why would we want this?} 
Following \citeA{Cimpian2010} we told participants one of five frequencies for each property (e.g. \emph{50\% of lorches have green feathers.}, and then asked for judgements of the corresponding generic sentence (e.g. \emph{Lorches have green feathers.}. 
...
\ndg{remind me why we can't compare speaker model with truth judgements, directly by item, as for the previous experiment?}
