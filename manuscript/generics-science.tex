...
Prevalence priors have two components: a density at 0\% prevalence, corresponding to all that categories for which the property is absent, and the remaining density, representing the prevalence of the property within the categories for which the property is present.
The relative mass equal to 0\% vs. greater than 0\% is a measure of \emph{between-category prevalence} (e.g. there are few or no animal categories used that have 0\% females, and thus \emph{are female} is a highly prevalent property \emph{between categories}).
...
Figure \ref{fig:priors1a} shows the between-category and within-category prevalence for all 21 different properties.
