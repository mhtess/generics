...
$L_1$ knows that the meaning of the generic utterance, if used, depends on a prevalence threshold, $\pi$, but does not know what this threshold is: $\tau \sim \text{Uniform([0,1])}$.
Speaker $S_1$ has a particular $\pi$ in mind, and is trying to be informative about the prevalence $x$ to the listener in his head---an idealized literal listener ($L_{0}$, Eq. \ref{eq:L0}).
The literal listener has access to the threshold $\pi$, and simply restricts her prior beliefs to situations where the truth-functional denotation of the utterance, $\denote{u}$, is true.
...
$P(x)=\theta \cdot \beta(\gamma,\xi)(x) + (1-\theta) \cdot \delta_{x=0}$.\footnote{This is similar in spirit to Hurdle Models of epidemiological data where the observed counts of zeros is substantially greater than one would expect from standard models of count data such as the Poisson model \cite<e.g. adverse events to vaccines; >{hurdleModels}.}
...
%To evaluate this model of the prior against our data, we inferred credible values for $\theta$, $\gamma$, and $\xi$ for each property using Bayesian statistical techniques (see Supplement Section A3).
...
\begin{figure}
\centering
    \includegraphics[width=0.8\columnwidth]{truthjudge-scatter-wLabels.pdf}
...
\subsubsection*{Interpreting novel generics} 
...
The pragmatic listener model (Eq.~\ref{eq:L1}) describes interpretation of a generic utterance, without previous knowledge of the prevalence within the target-kind. 
