...
The pragmatic listener in Eq.~\ref{eq:L1} is sensitive to the type of property (and its corresponding prior distribution on prevalence) when interpreting a novel generic.
...
\ndg{this paragraph has gotten too long.}
...
We also recruited participants ($n=40$) to help determine the average prevalence at which a speaker would assent to the generic. \ndg{why would we want this?} Following \citeA{Cimpian2010} we \ndg{?? told participants one of 4? frequencies for each property, and then asked for judgements of the corresponding generic sentence}. 
