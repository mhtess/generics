...
Logical quantifiers can be described as conditions on prevalence (i.e.~\emph{some} is $P(F|K)>0$, \emph{all} is $P(F|K)=1$). Extending this, it seems the simplest meaning for generic statements would be a similar threshold on prevalence: $P(F|K)>\theta$ \cite{cohen}. However, no fixed value of the threshold, $\theta$, would allow for the extreme flexibility generics exhibit. 
...
She has uncertainty about the prevalence of the property $x$ (``how many lions have manes?'') as well as the meaning of the generic, $\theta$. 
...
Following \cite{Degen}, we model felicity or truth judgements in terms of a speaker who has this sort of listener in mind:
