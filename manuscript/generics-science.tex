...
At first glance, generics seem like universally-quantified statements (i.e. ``All shoes have arch support''), but unlike universals, generics are resilient to counter-examples like \emph{Vibrams}. 
Interpreting the generic as meaning ``most'' (i.e. ``Most shoes have arch support.'') captures many cases, but cannot explain why ``Lions have manes'' and ``Mosquitos carry malaria.'' both seem true, despite the fact that among lions, only the adult males have manes and that a very small percentage of mosquitos actually carry malaria.
The only quantifier that could satisfy all of these conditions is the weak ``some'' (i.e. ``Some shoes have arch support.'').  
Yet, the semantics for ``some'' is not the semantics of generics:  Upon hearing a generic, listeners are wont to interpret the sentence as applying to \emph{all or nearly all} of a category \cite{Gelman2002, Cimpian2010}. 
``Some shoes have arch support.'' does not carry the same generalizability as ``Shoes have arch support.''.
...
\subsubsection{Begin experiment: 1400 words to this point} 
