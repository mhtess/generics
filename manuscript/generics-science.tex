...
At first glance, generics seem like universally-quantified statements (i.e. \emph{All swans are white}), but unlike universals, generics are resilient to counter-examples (e.g. \emph{black swans}). 
Interpreting the generic as meaning ``most'' (i.e. \emph{Most swans are white}) captures many cases, but cannot explain others: Only adult female robins lay eggs yet \emph{Robins lay eggs} is true and \emph{Mosquitos carry malaria} in spite of the fact that a very small percentage of mosquitos actually carry the virus.
Indeed, it appears that any explanation in terms of how common the property is within the category is incompatible with intuitions --- while \emph{Robins lay eggs} is fine, \emph{Robins are female} is not, even though the two properties are present in the same proportion (about 50\%) in the category of robins.
...
It seems that if \emph{Mosquitos carry malaria} is true, then the generic must in some way be analogous to ``some'' (i.e. \emph{Some swans are white.}). 
...
In this paper we offer a mathematical model of generic language in terms of pragmatic inference of the statistical degree required to assert a generic.  
...
We see that the interpretation of the generic depends a great deal on the shape of the prior, which can be summarized by the ``Property Distinctiveness'' (i.e. how common the property is \emph{across} different categories) and the ``Expected within-category prevalence'' (i.e. how common the property is expected to be \emph{within} a category, assuming the property is present; Figure \ref{fig:priors1a} scatterplot).
When the prior is very left-skewed (i.e. the property is highly distinct, as in the case of \emph{carries malaria}), then $\theta$ can plausibly be low while still being informative; if the prior is right-skewed (i.e. the property is not very distinct and is expected to be highly prevalent within-categories) a low threshold would not result in an informative utterance, and so would render the generic unlikely to be used by speaker $S_1$. The same general phenomenon can be observed for other nondistinctive properties and highly 
