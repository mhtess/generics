...
It seems that if \emph{Mosquitos carry malaria} is true, then the generic must mean ``some'' (i.e. \emph{Some apples are red.}). 
Yet generics have much stronger implications than ``some'': 
...
In fact, generic language seems to exaggerate how widespread the property is: The imputed prevalence is higher than the prevalence needed to assent to the generic, for both adults and children \cite{Cimpian2010, Brandone2014}. 

How can generics have such flexible \emph{truth conditions} while simultaneously have such strong \emph{implications}?
%\ndg{switch out lions for birds or something, because of the possibility that lions doesn't include lionesses.}
In this paper we offer a mathematical model of generic language in terms of pragmatic inference of the required degree of prevalence. We show that this model predicts the puzzlingly hetereogenous patterns, as well as quantitative detail, in human endorsement of generic sentences. It also captures the surprising d\'{e}colage \cite{Cimpian2010} between the high prevalence imputed after hearing a novel generic and the lower prevalence required to endorse the same generic.
It makes further predictions about how the imputed prevalence should vary as a function of beliefs about the property; these predictions are borne out.
...
\subsubsection{The semantics and pragmatics of generic language  (500 words to this point)}
...
\ndg{more about previous notions of the semantics of generics. eg motivate prevalence as a key component, mention prasada and leslie's notions of a special mental relation. then introduce RSA as formalizing something we know is in langauge and set up our model as the simple combination of prevalence with RSA. ie let's see how far we can get without a special kind of relation...}
 ...
 \mht{needs work}
At the most fundamental level, generics express a relation between a kind (e.g. \emph{robins}) and a property (e.g. \emph{lay eggs}). 
The nature of this relation is thought to depend, at least in part, on how prevalent the property is within the kind. For example, \emph{Lions lay eggs} is not true, because there are no lions that lay eggs. 
...
An alternative perspective emphasizes the structure of generic knowledge \cite{Prasada2000}, and views generic utterances as the way of expressing these special mental relationships between kinds and properties \cite{Leslie2008, Prasada2012}. 

We approach the problem of understanding generic language from a different starting point. 
...
We explore how the puzzles of generic language might be understood as the effects of pragmatic reasoning about a generic meaning that is underspecified in the language.
...
For a given kind, $K$ (e.g.~\emph{robins}), and a property, $F$ (e.g.~\emph{lay eggs}), we refer to the probability that an object of kind $K$ has property $F$, that is $P(F|K)$, as the \emph{prevalence} of $F$ within $K$
\footnote{Because we aim to explain the psycholinguistics of generics, we are generally interested in the subjective probability of an interlocutor, not the actual frequency in the world.}.
...
We suggest that generics do have a prevalence-threshold semantics, and that this threshold is established by pragmatic inference.
...
\end{eqnarray}
Eq.~\ref{eq:L1} models a listener ($L_{1}$) who has heard an utterance $u$, which is either a generic statement (e.g. ``Lions have manes.'') or an uninformative ``null'' utterance \footnote{All results reported are similar when the alternative is stipulated to be the negation,  ``It is not the case that lions have mains.'', as well as the generic of the negation, ``Lions do not have manes''.}.% These alternatives are analogous to alternatives to ``John is tall'' of ``John is not tall'' and ``John is short'' in the vague adjective case.}
She has uncertainty about the prevalence of the property $x \in [0, 1]$ (``how many lions have manes?'') as well as the meaning of the generic, $\theta$. 
...
As with other RSA models \cite{Lassiter2015}, the final value of $\theta$ balances truthfulness against informativity, and depends greatly on the prevalence prior, $P(x)$.
...
The probabilisitic model is fully specified by Eqs.~\eqref{eq:L1} -- \eqref{eq:S2}, with the exception of the speaker rationality parameter $\lambda$ and the prior distribution on prevalences $P(x)$. Because $\lambda$ is not of theoretical interest here, it is marginalized according to Bayesian data analysis principles \cite{LW2014}. 
...
\centering
    ...
    \caption{Truth judgments from for thirty generic utterances and the predictive distribution for the pragmatic model using empirical elicited priors. Order of the Y-axis corresponds to the rank-ordered model predictions. Error bars correspond with 95\% bootstrapped confidence intervals for the participant data and 95\% highest probability intervals for the model predictions.}
