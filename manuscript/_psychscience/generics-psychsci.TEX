...
In this series of experiments, we empirically measure the prior distribution over prevalence of properties of theoretical interest (e.g. \textsc{lays eggs}, \textsc{carries malaria}; Expt.~1a) and compare the predictions of the pragmatic speaker model $S_2$ using these measured priors to acceptability judgments of generic sentences about familiar kinds (Expt.~1b). 
...
To study this, we look to \emph{interpretation} of unfamiliar or novel generics in terms of how common the property is anticipated to be (e.g. \cite{Gelman2002, Cimpian2010}).
The pragmatic theoy we present includes such a theory of generic interpretation: the listener model (Eq.~\ref{eq:L1}) describes hearing a generic utterance----\emph{Feps \textsc{have property}}----without previous knowledge of the kind having the property.
...
In this series of experiments, we reveal a novel method for measuring \emph{a priori} beliefs about the prevalence of properties (Expt.~2a); compare the predictions of the pragmatic listener model $L_1$ using these empirically derived priors to interpretations of novel generic sentences (Expt.~2b); and use both the speaker $S_2$ and listener $L_1$ models to explain a previously reported empirical asymmetry between truth conditions and interpretations of generic sentences (Expt.~2c).


\section*{Experiment 2a: Prevalence priors for unfamiliar categories}

To measure the prevalence prior of properties for \emph{familiar} categories, participants filled out a table with rows corresponding to different animal kinds and columns corresponding to different properties (Expt. 1a). 
Without reference to a familiar kind, how can one know what prevalence to expect?
We revealed a latent structure to the prevalence priors task in Expt.~1a.
...
We used the same structured, statistical model 
The only difference from Expt.~1a. is that our experimental data comes from ask about the components of the priors directly (as opposed to only asking about the predicted prevalence within different familiar categories, as was done in Expt.~1a). 
%For Expt.~2a, participants are asked questions directly targeting $\theta$ and $\gamma$ in the above model (see {\it Expt. 2a}).
We assume these two measurements follow Beta distributions, and construct single prevalence distributions, $P(x)$, by sampling from the posterior predictive distribution of prevalence using the structured, statistical model as for Expt.~1a.
%
%\begin{minipage}{0.5 \textwidth} \small
\begin{align*}
...
%reflect a higher degree of uncertainty after hearing the generic utterance as compared to the biological properties. 
...
We recruited forty participants over MTurk to determine how widespread different properties are believed to be upon hearing a novel generic.  
Participants were restricted to those with US IP addresses and with at least a 95\% MTurk work approval rating. 
All participants were native English speakers. 
...
To understand more fully how the model makes these predictions, we performed two exploratory analyses to test the hypotheses that:
...
Both exploratory analysis have confirmatory evidence. 
...
\mht{Another trustworthy analysis shows Implied prevalence after hearing a generic is significantly greater than the \emph{a priori} prevalence}.
The generic does more than simply signal to generalize; it carries the communicative force of a speech-act.
...
Experimentally, \cite{Cimpian2010} has shown novel generics are interpreted as implying a higher prevalence than that required to assert the exact same generic.
We test both speaker $S_2$ and listener $L_1$ models in the same experiment following by adapting an analysis procedure from \cite{Cimpian2010}.
...
For both behavioral data and model predictions (Eq.~\ref{eq:S2}) we computed the average prevalence that led to an assenting judgment (the \emph{average truth conditions}), for each property type and participant, following the procedure used by \cite{Cimpian2010}.
...
The speaker model $S_2$ returns a posterior probability of saying the generic, for each level of prevalence\footnote{We assume here that the prevalence told to the participant is the subjective probability that that speaker $S_2$ is trying to communicate. This assumption is mostly likely wrong due to theory-driven considers. For example, if the participant believes the prevalence being reported in the experiment is describing a temporary or accidental state and not one that is likely to be true in the future, the speaker may derive a subjective probability substantially less than that stated verbally as fact in the experiment. Addressing this issue is beyond the scope of this article, and is not necessarily for the relatively coarse-grained metric of \emph{average truth conditions}}. 
