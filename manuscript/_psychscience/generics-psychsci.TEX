...
\section*{Computational method: The semantics and pragmatics of generic language}
...
In our case, we assume the speaker had the options of uttering the generic utterance or staying quiet---the \emph{null utterance} alternative carries no information (i.e.~$\denote{null}(x, \tau)=T$) and produces a posterior distribution in the listener identical with the prior.\footnote{
...
The contrast with the \emph{null} utterance alternative gives the generic it's communicative force, making it a \emph{speech-act}.
%
Throughout this paper we will treat the bare plural construction as generic utterances, hence they have a threshold semantics: $\denote{\text{K F}}(x, \tau)=x>\tau$.
The bare plural construction may additionally be ambiguous between a generic interpretation and a specific plural predication (consider \emph{dogs are on my lawn} compared to \emph{dogs have fur}); disambiguation itself may be a pragmatic process \cite{Cimpian2008}. 
...
%%
%%We model the interpretation of a generic utterance by $L_{1}$ (Eq. \ref{eq:L1}) as a pragmatic process 
%%resulting from the generic utterance (e.g.~\emph{Robins lay eggs.}) being produced by a speaker $S_{1}$ (Eq. \ref{eq:S1}), who had the alternative of not saying anything. 
...
\section*{Empirical puzzle 1: Flexible truth conditions}

A fundamental explanandum for a theory of generic language is that it explain the puzzling flexibility of truth conditions that generics exhibit.
It would seem that a theory based how prevalent the property is already has counterexamples: \emph{Birds lay eggs}, \emph{Mosquitos carry malaria}, and the infelicitousness of \emph{Birds are female}.
However, it is impossible to evaluate these examples without knowing 
what prior beliefs about these properties look like.  
%the backdrop (the priors) these sentences are evaluated against. 
In this series of experiments, we empirically measure the prior distribution over prevalence of properties of theoretical interest (e.g. \textsc{lays eggs}, \textsc{carries malaria}; Expt.~1a) and test the predictions of the pragmatic speaker model $S_2$ using these measured priors against human acceptability judgments of generic sentences about familiar kinds (Expt.~1b). 
...
Since this is a quantitative experiment with no planned comparisons, we chose this number of participants based on intuition and similar previous experiments.
...
\section*{Empirical puzzle 2: Strong interpretations}


From a learning perspective, the most important role for generic language is to provide information about new or poorly understood categories. 
To study this, we must try to understand how unfamiliar generic sentences are interpreted \cite<e.g.>{Gelman2002, Cimpian2010}.
The pragmatic theory we present includes such a theory of generic comprehension: the listener model (Eq.~\ref{eq:L1}) describes being told a generic utterance----\emph{Feps \textsc{have property}}----without previously knowing the kind had the property.
...
In this series of experiments, we reveal a novel method for measuring \emph{a priori} beliefs about the prevalence of properties (Expt.~2a); test the predictions of the pragmatic listener model $L_1$ using these empirically derived priors against human interpretations of novel generic sentences (Expt.~2b); and explain a previously reported empirical asymmetry between truth conditions and interpretations by comparing the speaker $S_2$ and listener $L_1$ models in the same experimental context (Expt.~2c).
...
In Expt.~1a, we discovered a latent structure to the prevalence priors task.
...
We harnessed this structure to measure prevalence priors with unfamiliar categories. 
...
Pilot testing revealed a more variability for items in the accidental category relative to the other types of properties. 
...
We used twice as many exemplars of accidental properties in order to more fully test the quantitative predictive power of the $L_1$ interpretation model, 
All materials are shown in Table S3.

Participants were introduced they were talking to a data-collection robot that was tasked with learning about properties of animals. 
Participants were told the robot randomly sampled an animal that it isn't knowledgable about to ask the participant about (e.g. The robots says: ``We recently discovered animals called feps.''). 
The robot then asks the participant two questions, aimed to measure to two components of the structured prior model: the potential of the property to be present in a kind and the expected prevalence when present.
The robot asked how likely it was that ``there was \emph{a} fep with \textsc{property}'' (potential to be present), to which participants on a scale from ``unlikely'' to ``likely''.
...
We assume these two measurements follow Beta distributions, and construct single prevalence distributions, $P(x)$, by sampling from the posterior predictive distribution of prevalence as we did before.
...
x & \sim \begin{cases} 
		...
		\end{cases} \\
\end{align*}
Biological properties are expected to be \emph{a priori} more prevalent within a kind when present than accidental properties, with fine-grained differences even among biological and accidental properties (Figure \ref{fig:prior2}).
...
Biological properties (``biological'', ``vague'', ``color'' body parts) have prevalence distributions that are bimodal with peaks at 0\% and near-100\% prevalence. 
...
This is characteristically strong interpretation that often receive \cite{Gelman2002}.
Accidental properties (``rare'' and ``common'') follow unimodal prior distributions and update to convex posterior distributions. 
...
We tested the degree to which the $L_1$ listener model, Eq.~\ref{eq:L1}, coupled with the empirically-elicited $P(x)$ from Expt.~2a predicted the degree to which the generic implied the property was widespread in the kind. 
...
This is a quantitative experiment with no planned comparisons.
...
\subsection*{Data--Model analysis and results}
...
%The model predicts 
We integrate out the model's 1 parameter the using the same Bayesian model analysis as before (details in SI Section D).
The pragmatic listener model predictions closely align with the human judgments of prevalence ($r^2(40)=0.89$, MSE=0.006), displaying the same sensitivity of interpretation to the property information (Figure \ref{fig:impliedByItem}). 
...
and (2) the prevalence implied by a generic is above the beyond the \emph{a proiri} expected prevalence (i.e. the generic is even stronger than ``just generalize'').
...
A mixed-effects linear model with random by-participant effects of intercept and random by-item effects of intercept and condition (prior prevalence vs. implied prevalence) reveals implied prevalence after hearing a generic is significantly greater than the \emph{a priori} prevalence ($\beta = 0.17; SE = 0.018; t(39) = 9.7; d = 0.64; p < 0.001$), providing evidence for (2).
The generic does more than simply signal to generalize; it carries with it the communicative weight of a speech-act.
...
It is often remarked that there is a surprising d\'{e}colage between the truth conditions and interpretations of generic language: Interpretations are characteristically strong while truth conditions are flexible. 
Experimentally, \citeA{Cimpian2010} have shown that novel generics are interpreted as implying a higher prevalence than that required to assert the exact same generic.
...
For both behavioral data and model predictions (Eq.~\ref{eq:S2}) we computed the average prevalence that led to an assenting judgment (the \emph{average prevalence score}), for each property type and participant, following the procedure used by \cite{Cimpian2010}.
...
Thus, our approach makes the strong claim that beliefs about prevalence are the connective tissue between conceptual knowledge and generic language.
