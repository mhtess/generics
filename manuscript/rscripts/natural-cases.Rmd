...
m<-read.csv(paste(m.path,
                  'generics-tj-prevalenceGammaUNIF20-incr', 
                  samples, 'a.csv', sep=''))
str(m)
...
m<-read.csv(paste(m.path,
                  ...
                  'generics-tj-prevalencePrior_hasFUNIF-has0-postpred-incr', 
                  samples, 'a.csv', sep=''))
...
m.sansHyper <- m %>% filter((Negation =='prob_hasF') | (Negation=='beta'))
...
m.params<- m %>% filter(Parameter 
                        %in% c("speakerOptimality", "cost", "softmax"))
