...
m<- read.csv(paste(m.path, 
                   "generics-truthJudgment-phi-previter10000_prevprioriter10000-priord50Zero-n60_mh50000_burn25000c.csv", 
                   sep=''))
...
m.prev<-m %>% filter(Parameter=='prevalence')
...
qplot(data=combined.model.data,
      x=posMode,y=response,
      color=Prevalence)+
  geom_abline(intercept=0,slope=1,linetype=2, size=1,color='grey69')+
  geom_errorbar(aes(ymin=YMin, ymax=YMax),
  ...
  geom_errorbarh(aes(xmin = credLo, xmax=credHi),
                  size = 1.5,
                 subset = .(Prevalence >=0.5))+
  geom_point(size=5, subset = .(Prevalence >=0.5))+
  geom_errorbar(aes(ymin=YMin, ymax=YMax),
                size =1.5,  position=position_dodge(),
                 subset = .(Prevalence <0.5))+
  geom_errorbarh(aes(xmin = credLo, xmax=credHi),
...
ggsave(file='~/Box Sync/talks/lightning-2015-generics/tj-scatter.pdf', width=12, height=10)
