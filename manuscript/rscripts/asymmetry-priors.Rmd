...
d.raw <- read.csv(file='~/Documents/research/generics/data/asymmetry/asymmetry-prior-4-trials.csv')

d.labels <- d.raw %>% rename(Parameter = stim_property) %>% select(Parameter, stim_type)

d.table<-left_join(d.out %>% filter(Item=='response0') %>% mutate(distinctiveness = 1-MAP,
                                               distinctHigh = 1-credLo,
                                               distinctLow = 1-credHi) %>%
            ungroup() %>%
  select(-expectation,-MAP, -credLo, -credHi, -Prevalence, -Item),
          d.out %>% filter(Item=='response1') %>% mutate(within = 1-MAP,
                                               withinHigh = 1-credLo,
                                               withinLow = 1-credHi) %>%
    ungroup() %>%
  select(-expectation,-MAP, -credLo, -credHi, -Prevalence, -Item))
...
d.table<- left_join(unique(d.labels),d.table) %>%
  mutate(stim_type = factor(stim_type, levels=c("part", "vague", "color", "accidental", "disease")))
...
write.csv(d.table, file='~/Documents/research/generics/manuscript/novelKinds-prevalencePrior-byItem-gammaCIs.csv', row.names=F)
