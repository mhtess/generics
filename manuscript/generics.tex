...
There is important latent structure in the prior elicitation task. We describe the model from the perspective of inferring the distribution of a particular property (e.g. ``lays eggs''). 
...
We make no assumptions about the amount of noise in the data, and we put uninformative priors over the means and concentrations of the prevalence distributions. 
...
We also inferred the likely prevalence of each animal category and property pair of interest, to be used as the measure of prevalence in the generic truth judgment task (Expt.~1b). 
...
By contrast, ``has manes'' is rare across categories, and has considerable  prevalence within-categories as ``is male'', reflected by the bump in density around 50\% (Figure \ref{fig:priors1a} bottom left).
