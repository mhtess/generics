...
\begin{abstract}
Generic utterances (e.g. ``Dogs bark'') express ideas about categories in the world and are ubiquitous in natural language. 
Despite their prevalence, the meanings of generic statements are puzzling to formal approaches. 
It is believed that generic utterances behave somewhat like quantified ones (e.g. ``Most dogs bark.''). If this were true, however, there should be some percentage of the category that must display the property (e.g. some threshold on the number of dogs that bark) for the utterance to be true.
This is notoriously difficult for generics, which can be true based on very little evidence but which can also imply near-universality of the property. 
Here, we formalize a semantics and pragmatics for generic language understanding: The threshold for acceptance is represented as an unknown property of the language and is actively reasoned about in context by a listener who assumes she is in conversation with an informative speaker. 
We explain how a simple, vague semantics can account for two central phenomena surrounding generic language: the conditions by which a generic is true and the variable implications of generic language. 
We do this using a novel behavioral and data-analytic technique for measuring beliefs about the prevalence of properties. 
...
We replicated \citeA{Cimpian2010}'s finding that the proportion of ``agree'' responses increases monotonically as prevalence increased ($\beta=0.05; SE = .003; z = 15.5$). 
