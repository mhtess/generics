...
Despite their centrality to cognitive development and everyday conversational, generic utterances have elusive meanings. % stable meanings for generic sentences are hard to come by.
...
\begin{flalign}
\denote{\text{Birds lay eggs}} = \{x^{\text{lays eggs}}_{bird} : x^{\text{lays eggs}}_{bird} = p(\text{lays eggs} | \text{bird}) > \theta_{generic}\} \label{eq:birds}
\end{flalign}
...
4 subjects were excluded for failing to successfully report which button corresponded with ``Agree''. 
...
The model has one free parameter, the speaker rationality in Equation \ref{eq:S1}. We put a uniform prior over this variable, and infer its credible values using a Bayesian statistical model. To learn about the \emph{a posteriori} credible values of our model parameter, we used the probabilistic programming language WebPPL \cite{dippl} to collect X samples using the Metropolis-Hastings algorithm. The estimated posterior density of this parameter is shown in Figure \ref{fig:rationality}. This parameter represents the subject's belief is how rational the hypothetical listener believes he is when choosing an utterance. It's important to note that a rationality parameter below 1 would imply behavior closer to random guessing. The 95\% Credible Interval is \red{[ , ]}, consistent with parameter values used in fitting other models of the same class.  
...
Marginal posterior distributions for $p$ for estimated using the Metropolis-Hastings algorithm in the probabilistic programming language WebPPL \cite{dippl}. Inference was completed by taking X samples with Y burn-in.

\subsubsection{Results}

Our focus for this experiment is to see if the distributions on prevalence differ between accidental and biological properties, and further to see if these differences predict the asymmetries observed in Expt.~2a. Thus, we collapse across items within each of these categories of property. 
