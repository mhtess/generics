...
The lifted-threshold RSA model predicts different truth conditions for generics when the prevalence prior differs. In Experiment 1a, we measure the distribution of prevalence of certain target properties by asking participants about the percentage of certain animal kinds with the property. So as to not bias the measurement in favor of a small number of stipulated animal categories, we ask participants to generate their own animal categories. In Experiment 1b, we measure the truth judgments of a number of generic statements about the properties measured in Expt. 1b. We show how point-wise prevalence is insufficient to explain the truth judgments of these generics. We then show how a model of a pragmatic listener who considers the distribution of prevalence across categories can explain a wide range of truth judgments.
...
There will be 8 properties in total. The properties will be revealed to you one at a time. You are allowed to go back and revise your answers, if you think there is a more realistic estimate you could give. 

\end{quote}

Throughout the course of the experiment, the participant filled out a table where each row was an animal category and each column was a property\footnote{The experiment in full can be viewed at \url{http://stanford.edu/~mtessler/experiments/generics/experiments/real-kinds/prior-1.html}}. Columns were revealed one at a time. First, participants saw a partially filled out column of animal kinds. These names were randomly sampled from a larger set. They were asked to add 5 of their own animal kinds to the list. Upon completion, the next column appeared with a property (e.g. ``lays eggs'') as the column header. Participants were asked: ``For each kind of animal, what percentage of the species do you think [property]?'' This procedure was repeated for 8 properties in total. The animal kind column was unable to modified once completed. The prevalence columns, however, could be modified at a later stage (i.e. the participant could go back and change her estimates after seeing more properties).

X properties were used in total. These comprised a set of properties associated with generics of theoretical interest, mapping onto certain conceptual distinctions. The conceptual categories used to generate the generics were: majority characteristic (e.g. ``Leopards have spots''), minority characteristic (e.g. ``Lions have manes''), striking (e.g. ``Sharks attack swimmers.''), and majority false generalizations (e.g. ``Ducks are female.'') \cite{Prasada2013}. The pre-specified animal names were those associated with these generics.  For a full list of the generic sentences used in this experiment, see Appendix \ref{sec:appendix}.
...
Experiment 2 attempted to replicate these findings of CBG: that there is an asymmetry between truth conditions (Expt. 2a) and implications (Expt. 2b) of the generic and that this asymmetry is sensitive to the type of property predicated. Further, in Expt.~2c, we measure participants' beliefs about the distribution of these types of properties using a similar paradigm to Expt.~1a. We show how the prevalence-based model predicts this asymmetry between truth conditions and implications, and how this asymmetry can be pushed around by the various properties predicated.

%Exp. 1a and 1b were conducted on separate sessions, one week apart. None of the participants completed both experiments.

\subsection{Experiment 2a: \emph{truth conditions}}
...
\subsection{Experiment 2b: \emph{implied prevalence}}
