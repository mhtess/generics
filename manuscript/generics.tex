...
\begin{abstract}
Generic utterances (e.g. ``Dogs bark'') are ubiquitous in natural language. Despite their prevalence, the meanings of generic statements are puzzling to formal approaches. A scalar semantic account stipulates a generic statement is true if the prevalence (the probability of the property given the category) is greater than some threshold. Such accounts are difficult to defend because of the inherent flexibility in generic truth conditions. Here, we propose a scalar semantics for generics wherein the truth conditional threshold is underspecified in the semantics but is determined by context and interaction. In Expts.~1, we replicate the findings of \citeA{Cimpian2010}, who provided strong empirical support for the intuition that generics statements have flexible truth conditions but are interpreted very strongly. The model predicts flexibility in truth conditions across different types of properties if the prior distribution over prevalence differs across these property-types; in Exp.~2 we find direct evidence that this is so. We conclude by showing that the model is able to capture accidental and low-prevalence generics---two cases of theoretical importance.
...
Imagine you're talking with a 3 year old about oxygen. Oxygen is difficult to explain because it's rarely observable and the function of oxygen is abstract. You might use a construction similar to the one from Simple Wikipedia above. Few would argue with the truth of this sentence (though perhaps if you were talking with an adult, you would use more specific language), yet the meaning is difficult to specify. Is it synonymous with ``All animals breathe Oxygen to live''? %It's false that \emph{all} animals breathe oxygen (e.g. fish), yet intuitively the sentence seems to convey something stronger than the weak \emph{some} animals breathe oxygen. 
...
This type of utterance is generic \cite{Carlson1977, Leslie2008} in that it conveys a generalization about a category. Generics can be true with little evidence (e.g. each of us has encountered only a small number of all the animal-kinds in the world, yet we would still endorse this). % should I deply the mosquitos here?
%
...
%
At the same time, generics are robust to counterexamples: ``Animals breathe Oxygen to live'' is true even though there are three species of creatures called \emph{lorica} that live 2.2 miles underwater on the ocean floor and apparently do not breathe oxygen to live. 
%
...
%
Finally, generics have strong implications. Upon hearing a generic, listeners are wont to interpret the sentence as applying to \emph{all or almost all} of a category.
%
...
%
With an uncertain semantics, the listener solves this problem by using her prior beliefs, in this case, about the prevalence of the property, in addition to standard inferences from conversational pragmatics \cite{Clark1996; Grice1975; Levinson2000}. From this, flexible truth conditions are derived. At the same time, near-universal implications are predicted if the prevalence of the property follows a certain distribution.
%
...
The prevalence prior $P(x)$ has a critical effect on the interpretation of the generic in this model. 
